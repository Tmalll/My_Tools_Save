...
const pushUpgradeRecipe = (oldTier, newTier, gemType) => {
  cubemain.rows.push({
    description: `Upgrade GEM ${config.gemUpgradeCount}x ${gemTiers[oldTier]} ${gemType.name} -> ${gemTiers[newTier]} ${gemType.name}`,
    enabled: 1,
    version: 100,
...
const pushDowngradeRecipe = (oldTier, newTier, gemType) => {
  ...
  cubemain.rows.push({
    description: `Downgrade GEM ${gemTiers[oldTier]} ${gemType.name} -> ${config.gemDowngradeCount}x ${gemTiers[newTier]} ${gemType.name}`,
    enabled: 1,
    version: 100,
    numinputs: 2,

...
misc.rows.forEach((row) =>
{
    ...
    if (config.allowGem) {
        // 如果开启了允许宝石
        if (
            (config.allowGemALL && row.type2.match(/^gem[0-4]$/) != null) // 匹配 gem0 ~ gem4
            || (!config.allowGemALL && row.type2.match(/^gem4$/) != null) // 只匹配 gem4
        )   {
                // 设置各个NPC商店的宝石数量
                row.AkaraMin = 1;
                row.AkaraMax = 1;
                ...
                row.MalahMin = 1;
                row.MalahMax = 1;
                row.PermStoreItem = 1; // 购买时不消失，永远有货

                // 如果开启 GemCost，则价格设置为 1
                if (config.GemCost) {
                    row.cost = 1;
                }
            }

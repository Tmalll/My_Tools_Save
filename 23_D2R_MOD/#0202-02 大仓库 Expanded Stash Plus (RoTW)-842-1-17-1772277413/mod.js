...

{
  ...
  processNodes(fileContent, (name, node) => {
    ...
    if (name === 'gold_withdraw') {
      ...
      node.fields.rect.y = -142;
    }

    if (name === 'gold_amount') {
      node.fields.rect.x = -220 + 50;
      node.fields.rect.y = -140 - 12;
    }

    if (name === 'gold_max') {
      node.fields.rect.x = 905 - 130;
      node.fields.rect.y = 52;//-150
      node.fields.rect.width = 150,
      node.fields.rect.height = 30;
    }
    ...
    if (name === 'StashNavigation' && node.fields && node.fields.rect) {
      node.fields.rect.x = 250;
      node.fields.rect.y = 1780;
    }

...
:: 1.Base64编码后的Token
set "TokenBase64=Z2l0aHViX3BhdF8xMUFKU1VXSlEwOGVRQ1hnZ1l0Mnc5X2cyc0d3TVU3SWV5dE1zSThpVjQ1a2o2RUlRVDZocEcwcnJFZ2dkc0NBbU5SSUdSWVFFWDZraTd1UEhaIA0K"
echo.

:: 2.还原Token并赋予变量
for /f "delims=" %%i in ('powershell -NoProfile -ExecutionPolicy Bypass -Command "[System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String('%TokenBase64%')).Trim()"') do set "TokenRestored=%%i"
echo.

:: 3.变量置换, 保留这个用于使用或调试固定Token...
set "TOKEN=%TokenRestored%"
echo %TOKEN%
echo.

:: 4.设置项目名称和仓库名称
set "OWNER=tuic-protocol"
set "REPO=tuic"


:: 5.指定 aria2-list.txt 的保存绝对路径
set "ARIA_PATH=%~dp0aria2-list.txt"

:: 6.自动组合仓库全名和创建Json存放文件夹
...
:: 8.构造 Authorization 请求头 (如果有 Token)
set "auth_header="
if not "%github_token%"=="" set "auth_header=-H "Authorization: Bearer %github_token%""

:: 9.提前定义好 URL 后缀，避开循环内 Exclamation (!) 符号被延迟扩展吞掉的巨坑

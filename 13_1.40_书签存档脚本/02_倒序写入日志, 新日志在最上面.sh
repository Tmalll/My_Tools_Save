...
# 记录日志并显示到最上面.
LOG_FILE_NAME=bak_BMS
LOG_FILE="$(cd "$(dirname "$0")"; pwd)/$LOG_FILE_NAME.log"
LOG_TMP="$(cd "$(dirname "$0")"; pwd)/$LOG_FILE_NAME.tmp"

finish() {
    if [ -f "$LOG_TMP" ]; then
        # 如果旧日志存在，则进行“新前旧后”的合并
        if [ -f "$LOG_FILE" ]; then
            # 先检查是否需要清理 7 天前的旧日志
            find "$LOG_FILE" -mtime +7 -exec cp /dev/null {} \; 2>/dev/null
            # 【关键】合并：新内容(TMP) + 旧内容(LOG) -> 新的LOG
            cat "$LOG_TMP" "$LOG_FILE" > "${LOG_FILE}.new"
...
# 3. 注册钩子：脚本退出时（无论成功还是报错）自动执行 finish
trap finish EXIT

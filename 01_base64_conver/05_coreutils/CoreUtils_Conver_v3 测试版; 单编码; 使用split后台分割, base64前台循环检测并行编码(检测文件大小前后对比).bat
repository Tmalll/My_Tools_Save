...
start /min "" cmd /c "coreutils.exe split -b %CHUNK_MB%M -d -a %PART_PAD% "%SRC%" "%DIR%%BASE%.part" && timeout /t 5 >nul && echo. > "%DIR%split.ok""

:toBase64Encode
:: 如果检测到 split.ok 文件，说明分割已完成，退出循环
if exist "%DIR%split.ok" goto :doneEncode

for /f "delims=" %%F in ('dir /b "%DIR%*.part*" ^| findstr /r "\.part[0-9][0-9][0-9]$"') do (
    if not exist "%DIR%%%F.b64" (
        :: 第一次取大小
        for %%S in ("%DIR%%%F") do set "s1=%%~zS"
        :: 等待200ms
        pathping -p 200 -q 1 localhost >nul
        :: 第二次取大小
        for %%S in ("%DIR%%%F") do set "s2=%%~zS"

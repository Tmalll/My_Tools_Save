...
"%PRpath%" -f "%~dp0\config.yml" -d "%configDIR%" -ext-ui "%configDIR%\webui" | grep -E --line-buffered -v "Auto detect interface|REJECT-DROP|GeoSite|using HidePROXY|using DNS_select|get empty name|interface not found|]:445|to avoid lookback|using DIRECT|may not have any sent data|i/o timeout|context deadline exceeded|can't resolve ip" > "%~dp0mihomo_RunLOG.log"
echo.

...
# ==========================================
# 在脚本顶部的配置区定义这个变量
$DebugMode = 1  # 1: 开启底层排查监控 | 0: 彻底关闭碍眼提示并停用激进 GC
# ==========================================
...
while ($true) {
    ...
    if ($logTimer.ElapsedMilliseconds -ge 10000) {
        if ($DebugMode -eq 1) {
            ...
            # 打印综合 Debug 信息
            [Console]::WriteLine("${esc}[93m[$timeStr] [SYSTEM MONITOR] Jobs: $($jobTracker.Count) | Heap: ${heapMB}MB | Private: ${privateMB}MB | WorkingSet: ${workingMB}MB${esc}[0m")
            
            # 智能 GC：仅在 Debug 模式且完全闲置时，拉长频率进行一次轻量回收
            if ($jobTracker.Count -eq 0) {
                [System.GC]::Collect(2, [System.GCCollectionMode]::Optimized)
            }
        }
        
        # 无论开不开 Debug，都重置计时器，防止无意义的 CPU 空转
        $logTimer.Restart()
    }

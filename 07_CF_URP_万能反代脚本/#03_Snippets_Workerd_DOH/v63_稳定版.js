const CONFIG = {
    ...
    // DOH 默认约束路径, 反代有此PATH的DOH进入DOH模块
    DOH_PATHS: new Set(['/dns-query', '/dns-query-7788']),

    // set_mime选项: 'ORIGIN' (保持源站) | 'AUTO_MAP' (后缀映射) | 强制指定MIME字符串 (如 text/html)
    // set_rewrite 智能覆写开关: 0: 关闭不覆写 | 1: 限有长度且<=max_size | 2: 允许<=max_size或无长度标头 | 3: 强制通通覆写
    // allow_doh 安全DNS开关:  0: 禁用 DOH | 1: 仅允许 GET (缓存) | 2: 仅允许 POST (不缓存) | 3: 同时允许 GET 和 POST
    // cache_ttl 缓存时间, 单位(秒).
    // ==================== 🛠️ 统一智能路由注册表 ====================
    ROUTE_REGISTRY: {
...
function getHelpHTML(origin) {
    const PANEL_TITLE = "CF 万能反代脚本 2026-06-22";
    const IS_JUMP = 1; // 页面链接默认跳转模式, 0 = 当前标签打开, 1 = 跳转新标签打开
    ...
    <div class="notice"><strong> DOH 风险：</strong> 使用CF-Wokers / Snippets 部署时, 反代 DOH 极易快速消耗响应次数, 导致项目被艹. 反代目标必须使用合法域名格式, 如: https://dns.google/dns-query，IP格式的可能会无法使用...</div>
    <script>

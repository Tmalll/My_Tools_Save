...
function parseRouteContext(pathname, prefixLength, searchAndHash) {
    ...
    if (REGEX_IMG_TAIL.test(cleanUrlForName)) cleanUrlForName = cleanUrlForName.substring(0, cleanUrlForName.lastIndexOf('/'));
...
export default {
    async fetch(request, env, ctx) {
        ...
        if (url.pathname === '/' + pAdmin || url.pathname === '/' + pAdmin + '/') {
            if (request.method === "POST") {
                try {
                    ...
                    let cleanUpstream = pureUrl.replace('://', '/');
                    if (REGEX_IMG_TAIL.test(cleanUpstream)) cleanUpstream = cleanUpstream.substring(0, cleanUpstream.lastIndexOf('/'));
                    ...
                    let tail = `/image.${ext}`;
                    let disUrl = `${origin}/${makeFullUrlPart(cleanUpstream, pShare)}${tail}`;
        ...
        let browserHasImgTail = REGEX_IMG_TAIL.test(url.pathname);
        if (url.pathname.startsWith('/' + pRef + '/') || url.pathname.startsWith('/' + pSho + '/')) {
            ...
            if (ctx.registryResult) return Response.redirect(`${url.origin}/${isRef ? pSimp : ctx.registryResult.site.aliasPrefix.replace(/^\//,'')}/${isRef ? ctx.registryResult.ref_URL.replace('://', '/') : ctx.registryResult.ID}${ctx.tail}`, 302);
            if (!isRef) {

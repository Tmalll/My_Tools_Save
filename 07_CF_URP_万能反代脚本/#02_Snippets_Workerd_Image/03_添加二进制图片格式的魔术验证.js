...
// 2. 新增：原始二进制图片魔数白名单 (用于彻底封杀假冒 Content-Type)
const RAW_MAGIC_REGISTRY = [
    { mime: "image/png",      ext: "png",  bytes: [0x89, 0x50, 0x4E, 0x47] },             // .png
    { mime: "image/jpeg",     ext: "jpg",  bytes: [0xFF, 0xD8, 0xFF] },                   // .jpg / .jpeg
    { mime: "image/gif",      ext: "gif",  bytes: [0x47, 0x49, 0x46, 0x38] },             // .gif (GIF8)
    { mime: "image/webp",     ext: "webp", bytes: [0x52, 0x49, 0x46, 0x46] },             // .webp (RIFF..., 后面第8-12字节是WEBP)
    { mime: "image/x-icon",   ext: "ico",  bytes: [0x00, 0x00, 0x01, 0x00] },             // .ico
    { mime: "image/bmp",      ext: "bmp",  bytes: [0x42, 0x4D] },                         // .bmp (BM)
    { mime: "image/svg+xml",  ext: "svg",  bytes: [0x3C, 0x3F, 0x78, 0x6D] },             // .svg (<?xm)
    { mime: "image/svg+xml",  ext: "svg",  bytes: [0x3C, 0x73, 0x76, 0x67] }              // .svg (<svg)
];
...
export default {
    async fetch(request, env, ctx) {
        ...
        // 1. 应对普通图片直链：拦截 Content-Type 为图片的请求，进行二进制字节码硬核魔数比对
        if (RAW_IMAGE_MIME_WHITELIST.includes(cleanMime) || cleanMime === "application/octet-stream") {
            const reader = upstream.body.getReader();
            const { done, value } = await reader.read();
            
            if (done || !value) {
                return new Response("Forbidden: Empty or Corrupted Stream", { status: 403 });
            }

            // 核心：比对读出的第一个 Chunk 字节内容
            let isRealImage = false;
            let detectedMime = cleanMime;

            for (const item of RAW_MAGIC_REGISTRY) {
                if (value.length >= item.bytes.length) {
                    const match = item.bytes.every((b, i) => value[i] === b);
                    if (match) {
                        isRealImage = true;
                        detectedMime = item.mime; // 纠正假冒或泛型的 Content-Type
                        break;
                    }
                }
            }

            // 特殊放行：针对无法简单靠前 4 字节判断的 avif 格式补漏 (包含 ftypavif 签名)
            if (!isRealImage && cleanMime === "image/avif") {
                const textChunk = new TextDecoder().decode(value.subarray(0, 30));
                if (textChunk.includes("ftypavif") || textChunk.includes("ftypavis")) {
                    isRealImage = true;
                }
            }

            // 如果字节码魔数不符合任何已知图片规范，哪怕 Content-Type 写着 image 也是假的！直接 403 阻断！
            if (!isRealImage) {
                return new Response("Forbidden: Binary Magic Check Failed. Asset Is Tampered.", { status: 403 });
            }

            // 【黑科技：无损流式重建】验证通过后，将读出的第一个 chunk 和原流剩下的部分完美合并，继续流式下发
            const remainderStream = new ReadableStream({
                start(controller) {
                    controller.enqueue(value); // 把刚才用来读魔数的 chunk 吐回流里
            ...
            respHeaders.set("Content-Type", detectedMime); // 强制规范 Content-Type

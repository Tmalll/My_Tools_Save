...
const ROUTERS = [
    ...
    { match: (u, state) => u.pathname.startsWith('/' + PREFIX.dis + '/') && u.pathname.includes('://'), handler: async (req, u) => {
        let clean = `/${PREFIX.dis}/${u.pathname.replace(/\/+$/, '').replace(/^https?:\/\/[^\/]+\/dis\//i, '').replace('://', '/')}`.replace(/^\/dis\/dis\//i, '/dis/');
        return redirect(u.origin, `${clean}${u.search}${u.hash}`);
    }},

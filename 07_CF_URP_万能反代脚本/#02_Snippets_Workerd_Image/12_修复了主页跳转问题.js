const CONFIG = {
    ...
    SHARE_PREFIX: '/dis',
    REFINING_PREFIX: '/ref', REFINING_OUT_PREFIX: '/simp', 
    ShortName_PREFIX: '/sho',
    hide64_PREFIX: '/hid', DECODE_B64_PREFIX: '/dcb',
        ...
        if (url.pathname === '/' + pAdmin || url.pathname === '/' + pAdmin + '/') {
            if (request.method === "POST") {
                try {
                    ...
                    let isB64Tail = pureUrl.toLowerCase().endsWith('.b64');
                    let tail = "";
                    if (pureUrl.toLowerCase().match(/\.(jpg|jpeg|png|gif|webp|ico|bmp|svg|avif)/)) {
                        let m = pureUrl.match(/\/([^\/]+\.[a-z0-9]+)$/i);
                        if (m) tail = '/' + m[1];
                    }
                    if (isB64Tail) tail = '/image.jpg';

                    let disUrl = `${origin}/${makeFullUrlPart(pureUrl, pShare)}${isB64Tail && !tail ? '/image.jpg' : tail}`;
                    let links = { "明文直链反代 (DIS)": disUrl };

                    let regRes = parseRefRegistry(pureUrl);
                    if (regRes) {
                        let useTail = isB64Tail ? "/image.jpg" : tail;
                        if (!useTail || useTail.includes('.b64')) useTail = "/image.jpg";
                        links["托管精炼反代 (SIMP)"] = `${origin}/${pSimp}/${regRes.site.ref_URL.replace('://', '/')}${regRes.ID}${useTail}`;
                        links["特征短链反代 (SHORT)"] = `${origin}/${regRes.site.shortName}/${regRes.ID}${useTail}`;
                    }
...
<head>
    ...
    <style>
        ...
        .input-row, .btn-row { display: flex; gap: 10px; margin-bottom: 15px; width: 100%; }
        input[type="text"], select.mode-select { flex: 1; padding: 14px; font-size: 14px; border: 1px solid #ccc; border-radius: 4px; box-sizing: border-box; outline: none; }
        input[type="text"]:focus, select.mode-select:focus { border-color: #409eff; }
        .btn { padding: 14px; font-size: 14px; color: #fff; background: #409eff; border: none; border-radius: 4px; cursor: pointer; font-weight: bold; text-align: center; box-sizing: border-box; flex: 1; }

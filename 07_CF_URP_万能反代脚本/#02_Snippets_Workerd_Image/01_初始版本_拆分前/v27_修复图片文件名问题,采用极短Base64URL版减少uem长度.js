...
// 模块9：高强度密码学加解密与高效 Base64URL 压缩管道
...
async function encryptStealthText(plainText, seed) {
    const ctx = await getStealthCryptoContext(seed), enc = new TextEncoder(), data = enc.encode(plainText), encrypted = await crypto.subtle.encrypt({ name: 'AES-GCM', iv: ctx.iv }, ctx.key, data);
    return btoa(String.fromCharCode(...new Uint8Array(encrypted))).replace(/=/g, '').replace(/\+/g, '-').replace(/\//g, '_');
}
async function decryptStealthText(b64UrlText, seed) {
    try {
        const ctx = await getStealthCryptoContext(seed), s = b64UrlText.replace(/-/g, '+').replace(/_/g, '/'), pad = (4 - (s.length % 4)) % 4, rawStr = atob(s + "=".repeat(pad)), arr = new Uint8Array(rawStr.length);
        for (let i = 0; i < rawStr.length; i++) arr[i] = rawStr.charCodeAt(i);
        const decrypted = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: ctx.iv }, ctx.key, arr);
...
export default {
    async fetch(request, env, ctx) {
        ...
        const url = new URL(request.url), pAdmin = CONFIG.AUTH_PREFIX.replace(/^\/+|\/+$/g, ''), pShare = CONFIG.SHARE_PREFIX.replace(/^\/+|\/+$/g, ''), pRef = CONFIG.REFINING_PREFIX.replace(/^\/+|\/+$/g, ''), pSho = CONFIG.ShortName_PREFIX.replace(/^\/+|\/+$/g, ''), pHid = CONFIG.hide64_PREFIX.replace(/^\/+|\/+$/g, ''), pDcb = CONFIG.DECODE_B64_PREFIX.replace(/^\/+|\/+$/g, ''), pUhm = CONFIG.UHM_PREFIX.replace(/^\/+|\/+$/g, ''), pUem = CONFIG.UHM_OUTPUT.replace(/^\/+|\/+$/g, ''), cleanPath = url.pathname.toLowerCase();
        ...
        if (url.pathname.startsWith('/' + pUhm + '/')) {
            let targetPath = url.pathname.substring(pUhm.length + 2), rawTargetUrl = parseTargetUrl(targetPath + url.search + url.hash);
            if (!rawTargetUrl) return new Response("Invalid Target URL for UHM Mode", { status: 400 });
            const res = tryRefiningUrlEx(rawTargetUrl.toString());
            let base64UrlPayload = "", origName = "", checkExt = "";
            if (res) {
                base64UrlPayload = btoa(`${res.siteConfig.shortName}/${res.id}`).replace(/=/g, '');
                origName = cleanOriginalName(res.webFileName);
                checkExt = origName.split('.').pop().toLowerCase();
            } else {
                ...
                origName = cleanOriginalName(lastSl !== -1 ? cleanUrlPart.substring(lastSl + 1) : "image.png");
                checkExt = origName.split('.').pop().toLowerCase();
            }
            const isTargetImg = MAGIC_REGISTRY.some(m => m.isImage && (m.ext === checkExt || origName.toLowerCase().endsWith('.' + m.ext)));
            const outputName = isTargetImg ? `image.${checkExt || 'png'}` : (origName || "image.png");
            const encryptedUrl = await encryptStealthText(base64UrlPayload, CONFIG.STEALTH_KEY);
            return Response.redirect(`${url.origin}/${pUem}/${encryptedUrl}/${outputName}`, 302);
        }
        if (url.pathname.startsWith('/' + pHid + '/')) {
            ...
            const res = tryRefiningUrlEx(rawTargetUrl.toString());
            if (res) {
                const encPart = btoa(`${res.siteConfig.shortName}/${res.id}`).replace(/=/g, ''), origName = cleanOriginalName(res.webFileName);
                return Response.redirect(origName ? `${url.origin}/${pDcb}/${encPart}/${origName}` : `${url.origin}/${pDcb}/${encPart}/image.png`, 302);
            } else {
                let cleanUrlPart = rawTargetUrl.toString();
                if (/^https?:\/\//i.test(cleanUrlPart)) cleanUrlPart = cleanUrlPart.replace(/^https?:\/\//i, m => m.toLowerCase().replace('://', '/'));
                const encPart = btoa(`${pShare}/${cleanUrlPart}`).replace(/=/g, ''), lastSl = cleanUrlPart.lastIndexOf('/'), origName = cleanOriginalName(lastSl !== -1 ? cleanUrlPart.substring(lastSl + 1) : "image.png");
                return Response.redirect(`${url.origin}/${pDcb}/${encPart}/${origName}`, 302);
            }
        }
        if (url.pathname.startsWith('/' + pSho + '/')) {
            ...
            if (res) {
                ...
                return Response.redirect(origName ? `${url.origin}/${res.siteConfig.shortName}/${res.id}/${origName}` : `${url.origin}/${res.siteConfig.shortName}/${res.id}/image.png`, 302);
            } else {
                let cleanUrlPart = rawTargetUrl.toString();
                if (/^https?:\/\//i.test(cleanUrlPart)) cleanUrlPart = cleanUrlPart.replace(/^https?:\/\//i, m => m.toLowerCase().replace('://', '/'));
        ...
        if (url.pathname.startsWith('/' + pUem + '/')) {
            const rawPart = url.pathname.substring(pUem.length + 2), firstSlash = rawPart.indexOf('/'), cryptPart = firstSlash !== -1 ? rawPart.substring(0, firstSlash) : rawPart;
            const decryptedB64 = await decryptStealthText(cryptPart, CONFIG.STEALTH_KEY);
            if (!decryptedB64) return new Response("Forbidden: Invalid or Tampered UHM Payload", { status: 403 });
            try {

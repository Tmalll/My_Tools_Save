...
// 模块9：密码学对称加解密底座引擎（SHA256固定派生 Key 和 IV 保证强缓存绝对一致）
...
async function decryptStealthText(hexText, seed) {
    try {
        const ctx = await getStealthCryptoContext(seed), arr = new Uint8Array(hexText.match(/.{1,2}/g).map(byte => parseInt(byte, 16))), decrypted = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: ctx.iv }, ctx.key, arr);
...
export default {
    async fetch(request, env, ctx) {
        ...
        if (url.pathname.startsWith('/' + pUhm + '/')) {
            ...
            return Response.redirect(`${url.origin}/${pUem}/${encryptedUrl}/${origName || "image.png"}`, 302);
        }

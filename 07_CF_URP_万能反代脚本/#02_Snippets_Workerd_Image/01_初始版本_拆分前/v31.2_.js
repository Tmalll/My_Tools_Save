...
function parseBinaryName(urlStr) {
    ...
    // 如果尾巴本来就是图片包装后缀，先把它切掉，暴露出真实的文件名
...
// 辅助工具：深度递归 URL 解码（解决被重定向或多次编码后的双重编码 %25E6 乱码硬伤）
...
export default {
    async fetch(request, env, ctx) {
        ...
        if (url.pathname.startsWith('/' + pRef + '/')) {
            ...
            if (registryResult) {
                // 将原文件名作为尾巴保留：/dis/https/gist.githubusercontent.com/raw/id/BinaryName
        ...
        if (url.pathname.startsWith('/' + pSho + '/')) {
            ...
            let registryResult = parseRefRegistry(fullUrl);
            if (registryResult) {
                // 拼接得到短路由并原汁原味带上文件名尾巴：${GIS}/${ID}/${BinaryName}
        ...
        if (cryptoMode >= 0) {
            ...
            // 明文尾巴完美拼接在加密 URL 的后面，确保文件名在多次分发中也绝对能找回
        ...
        let incomingBinaryName = parseBinaryName(url.pathname); // 全局首先记录浏览器里的明文尾巴文件名
        ...
        // 3. 核心清洗剥离：不管后面带了什么长尾文件名，fetch 上游官方直链时一律清洗干净！
        let pureUpstreamPath = workingPath.substring(pShare.length + 2);
        let parts = pureUpstreamPath.split('/');
        
        // 判定如果是 Gist 或者是 Pastebin 的官方直链格式，强行截取保留前 4 段（协议、域名、raw、ID），剥离长尾！
        ...
        if (matchedMeta.isImage) {
            ...
            // 【核心修复】：执行深度递归 URL 解码，彻底干掉因多次转发引起的 %25E6 双重编码，还原真正的中文汉字！
            ...
            if (CONFIG.ALLOW_FALLBACK_DOWNLOAD !== 1) return new Response("Forbidden: Blocked asset.", { status: 403 });

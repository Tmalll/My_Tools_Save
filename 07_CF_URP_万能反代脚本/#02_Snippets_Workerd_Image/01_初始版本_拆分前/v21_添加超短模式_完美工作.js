...
const REFINING_REGISTRY = [
    {
        ...
        match_group: [
            ...
            { regex: /gist\.githubusercontent\.com\/[^\/]+\/([0-9a-f]{32})\/raw/i },
            // 8. usercontent 仅有用户名和ID，无末尾raw的链接 (修复漏掉的场景)
...
export default {
    ...
    async fetch(request, env, ctx) {
        ...
        // 3. 【超简短路由中转层】：如果是 /sho/ 模式，去 ref 走一圈，直接组装重定向到外部可见的别名短地址
        ...
        // 4. 【精炼中转站路由】：如果是 /ref/ 模式，保持原有逻辑稳定运行
        ...
        // 5. 【别名路由映射器】：隐式转换 /gis/ 或 /pas/ 到原生的 /dis/ 分流逻辑中，成为 dis 的无感别名
        ...
        if (isShortAliasRoute && matchedAliasConfig) {
            // 将类似 "/gis/ID/image.png" 的路径
            // 重新隐式拼装回底层的 "/dis/https/gist.githubusercontent.com/raw/ID/image.png"
            const remainingPart = url.pathname.substring(matchedAliasConfig.shortName.length + 2);
            const rawUrlPrefix = matchedAliasConfig.ref_URL.replace('://', '/');
            activePathname = '/' + pShare + '/' + rawUrlPrefix + remainingPart;
        }

        // 6. 安全前缀路由鉴权校验 (兼容原生 dis 与别名路由)
        ...
        if (matchedMeta.isImage) {
            ...
            // 二进制模式处理 (基于映射修正后的目标源站路径提取原始文件名，确保真实性)

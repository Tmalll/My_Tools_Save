...
// 模块7：核心文件名决策引擎（图片锁定格式，二进制循环解URL编码并裁切后缀）
...
export default {
    async fetch(request, env, ctx) {
        ...
        const url = new URL(request.url), pAdmin = CONFIG.AUTH_PREFIX.replace(/^\/+|\/+$/g, ''), pShare = CONFIG.SHARE_PREFIX.replace(/^\/+|\/+$/g, ''), pRef = CONFIG.REFINING_PREFIX.replace(/^\/+|\/+$/g, ''), pSho = CONFIG.ShortName_PREFIX.replace(/^\/+|\/+$/g, ''), pHid = CONFIG.hide64_PREFIX.replace(/^\/+|\/+$/g, ''), pDcb = CONFIG.DECODE_B64_PREFIX.replace(/^\/+|\/+$/g, ''), cleanPath = url.pathname.toLowerCase();
        ...
        if (url.pathname.startsWith('/' + pDcb + '/')) {
            ...
            try {
                let decodedStr = atob(base64UrlParam).trim(); const firstPart = decodedStr.indexOf('/');
                if (firstPart !== -1) {

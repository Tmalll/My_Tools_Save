...
const CONFIG = {
    ...
    STEALTH_KEY: 'Tmalll_Secret_Salt_2026', // UHM密钥种子
    DEBUG_MODE: 1,
    CACHE_TTL: 2592000,
    BACKEND_MAX_SIZE_KB: 10240,     // 遗留参数, 后端解码最大大小(KB)
    ALLOW_BACKEND_CHUNKED_RAW: 1,   // 遗留参数, 是否允许后端解码无长度(Transfer-Encoding: chunked)的RAW数据 [1:允许 | 0:锁定必须有长度]
...
export default {
    async fetch(request, env, ctx) {
        ...
        if (isNativeDisMode && url.pathname.includes('://')) {
            ...
            // 携带原 search 结构直接重定向到规范化路径，从而完美覆盖图片与二进制

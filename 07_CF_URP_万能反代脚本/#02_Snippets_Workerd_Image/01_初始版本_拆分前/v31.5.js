    ...
    async fetch(request, env, ctx) {
        ...
        if (matchedMeta.isImage) {
            ...
            if (!url.pathname.toLowerCase().endsWith('/' + expectedTail)) {
                ...
                if (isNativeDisMode) {
                    // 原生普通 dis 模式：强制将首位规范化处理，完全杜绝连续双 dis 的出现
                    ...
                    // 核心修正：拼装后通过正则把所有的 /dis/dis/ 彻底缩减恢复为单个 /dis/
                    let cleanPath = `/${pShare}/${currentUrlPath}`.replace(/\/+/g, '/');
                    cleanPath = cleanPath.replace(/^\/dis\/dis\//i, '/dis/');

...
function normalizeUrl(urlStr) {
    let raw = urlStr.trim(), proto = "https";
    let m = raw.match(REGEX_HTTP);
    if (m) { proto = m[1].toLowerCase(); raw = raw.replace(REGEX_HTTP, ''); }
    else if (raw.startsWith('http/')) { proto = 'http'; raw = raw.substring(5); }
    else if (raw.startsWith('https/')) { proto = 'https'; raw = raw.substring(6); }
    let firstSlashIdx = raw.indexOf('/');
    if (firstSlashIdx === -1) return proto + '://' + raw.replace(/\/+/g, '/');
    let hostPart = raw.substring(0, firstSlashIdx).replace(/\/+/g, '/'), pathPart = raw.substring(firstSlashIdx);
    if (hostPart === 'gist.githubusercontent.com' && pathPart.startsWith('/https/')) pathPart = pathPart.substring(6);
    return proto + '://' + hostPart + pathPart;
}
...
function parseRouteContext(pathname, prefixLength, searchAndHash) {
    ...
    let cleanUrlForName = fullUrl.replace(/\/+$/, '').split('?')[0];
...
function checkHasValidExt(pathStr) {
    let clean = pathStr.replace(/\/+$/, '').split('?')[0].toLowerCase();
...
function makeFullUrlPart(urlStr, pShare) {
    let c = urlStr.trim();
    let m = c.match(/^(https?):\/\//i);
    if (m) { c = m[1].toLowerCase() + '/' + c.replace(/^(https?):\/\//i, ''); }
...
function bytesToBase64Url(arr) {
    ...
    for (let i = 0; i < len; i += chunk) {
        buf.push(String.fromCharCode.apply(null, bytes.subarray(i, i + chunk)));
    }
...
async function handleAdminRoute(request, url, prefixes) {
    ...
    if (request.method === "POST") {
        try {
            ...
            let regRes = parseRefRegistry(cleanUpstream);
            if (regRes) {
                links["托管精炼反代 (SIMP)"] = `${origin}/${pSimp}/${regRes.site.ref_URL.replace(/^(https?):\/+/i, '$1/')}${regRes.ID}${tail}`;
            ...
            const encTasks = [encryptStealthText(b64Payload, CONFIG.STEALTH_KEY, 1), encryptStealthText(b64Payload, CONFIG.STEALTH_KEY, 2), encryptStealthText(b64Payload, CONFIG.STEALTH_KEY, 3)];
            const encResults = await Promise.all(encTasks);
            for (let i = 1; i <= 3; i++) {
                links[`AES加密混淆路由 (UHM${i})`] = `${origin}/${pUem[i-1]}/${encResults[i-1]}${tail}`;
            }
...
function handleRefAndShortRoute(url, pRef, pSho, pSimp, pShare) {
    ...
    if (ctx.registryResult) {
        let nPath = isRef ? `/${pSimp}/${ctx.registryResult.site.ref_URL.replace(/^(https?):\/+/i, '$1/')}${ctx.registryResult.ID}` : `/${ctx.registryResult.site.aliasPrefix.replace(/^\//,'')}/${ctx.registryResult.ID}`;
        return { redirect: true, url: `${url.origin}${nPath.replace(/\/+/g, '/')}${url.search}${url.hash}` };
    }
    if (!isRef) {
        if (/^[0-9a-f]{32}$/i.test(cleanId)) return { redirect: true, url: `${url.origin}${BASE_AUTH_PREFIX}/gis/${cleanId}${url.search}${url.hash}`.replace(/\/+/g, '/') };
        if (/^[a-zA-Z0-9]{8}$/.test(cleanId)) return { redirect: true, url: `${url.origin}${BASE_AUTH_PREFIX}/pas/${cleanId}${url.search}${url.hash}`.replace(/\/+/g, '/') };
    }
    let fallbackPart = makeFullUrlPart(ctx.fullUrl, pShare);
    return { redirect: true, url: `${url.origin}/${fallbackPart}${url.search}${url.hash}`.replace(/\/+/g, '/') };
}
async function handleCryptoRedirectRoute(url, pHid, pUhm, pDcb, pUem, pShare) {
    ...
    return `${url.origin}/${cryptoMode === 0 ? pDcb : pUem[cryptoMode - 1]}/${encryptedPayload}${url.search}${url.hash}`;
}
async function handleMainImageProxy(request, url, workingPath, pShare, ctx, browserHasImgTail) {
    ...
    let upstreamUrlStr = m ? m[1] + '://' + m[2] : 'https://' + pureUpstreamPath;
    let upstreamUrl = null;
    try { upstreamUrl = new URL(upstreamUrlStr + url.search); } catch { return new Response("Invalid Upstream Target", { status: 400 }); }
    ...
    try { firstChunkObj = await reader.read(); } catch(e) { return new Response(`Stream Read Error: ${e.message}`, { status: 502 }); }
    let { done, value: firstChunk } = firstChunkObj;
    if (done || !firstChunk) return new Response("Forbidden: Empty Stream Source.", { status: 403 });
    if (firstChunk.length > maxBackendBytes) { try { await reader.cancel(); } catch{} return new Response(`Forbidden: Stream overflow.`, { status: 413 }); }
    let isRealImage = false, detectedMime = "application/octet-stream", foundExt = "", isB64Mode = false;
    for (const item of MAGIC_REGISTRY) {
    ...
    if (!isRealImage) { try { await reader.cancel(); } catch{} return new Response("Forbidden: Non-image assets blocked.", { status: 403 }); }
    if (!browserHasImgTail) { try { await reader.cancel(); } catch{} return Response.redirect(`${url.origin}${url.pathname.replace(/\/+$/, '')}/image.${foundExt}${url.search}${url.hash}`, 302); }
    if (!isB64Mode) {
            ...
            async start(controller) {
                ...
                try {
                    while (true) {
                        ...
                        if (totalBytesRead > maxBackendBytes) {
                            hasError = true;
                            try { await reader.cancel(); } catch{}
                            controller.error(new Error("Forbidden: Stream overflow."));
                            break;
                        }
        ...
        return makeImageResponse(bodyStream, detectedMime, null, cacheKey, cache, ctx);
    }
    ...
    try {
        while (true) {
            let { done, value } = await reader.read();
            if (done) break;
            if (totalBytesRead + value.length > maxBackendBytes) { try { await reader.cancel(); } catch{} return new Response(`Forbidden: Stream overflow.`, { status: 413 }); }
            chunks.push(value); totalBytesRead += value.length;
        }
    } catch(e) { return new Response(`Stream Body Interrupted: ${e.message}`, { status: 502 }); }
...
export default {
    async fetch(request, env, ctx) {
        ...
        if (url.pathname.startsWith('/' + pRef + '/') || url.pathname.startsWith('/' + pSho + '/')) {
            let routeRes = handleRefAndShortRoute(url, pRef, pSho, pSimp, pShare);
        ...
        if (activeAlias) workingPath = '/' + pShare + '/' + activeAlias.ref_URL.replace(/^(https?):\/+/i, '$1/') + workingPath.substring(activeAlias.aliasPrefix.length + 1).split('/')[0];
        ...
        if (url.pathname.startsWith('/' + pShare + '/') && url.pathname.includes('://')) {
            let cleanPath = `/${pShare}/${url.pathname.replace(/\/+$/, '').replace(/^https?:\/\/[^\/]+\/dis\//i, '').replace('://', '/')}`.replace(/^\/dis\/dis\//i, '/dis/');
...
<body>
    ...
    <script>
        ...
        document.getElementById('fileFile').onchange = function() {
            ...
            const s = 1024 * 256; let o = 0, chunks = [], td = new TextDecoder("latin1");
            const r = () => {
                const reader = new FileReader(), blob = f.slice(o, o + s);
                reader.onload = function(e) {
                    chunks.push(td.decode(new Uint8Array(e.target.result))); o += s;

...
export default {
    async fetch(request, env, ctx) {
        ...
        if (!url.pathname.toLowerCase().endsWith('/' + expectedTail)) {
            let redirectPath = isNativeDisMode ? `/${pShare}/${url.pathname.replace(/\/+$/, '').replace(/^\/dis\/dis\//i, '/dis/')}/${expectedTail}` : request.url.replace(/\/+$/, '').split('/');
            if (!isNativeDisMode) {
                if (redirectPath[redirectPath.length - 1].toLowerCase().endsWith('.b64') || redirectPath[redirectPath.length - 1].toLowerCase().endsWith('.base64') || redirectPath[redirectPath.length - 1].toLowerCase().startsWith('image.')) redirectPath.pop();
                redirectPath = redirectPath.join('/') + '/' + expectedTail;
            } else {
                redirectPath = url.origin + redirectPath;
            }

const CONFIG = {
    ...
    BACKEND_MAX_SIZE_KB: 10240,      // 后端图片大小限制(KB)
    ALLOW_BACKEND_CHUNKED_RAW: 1     // 是否允许后端处理未标记长度的流式分块数据
};
...
export default {
    async fetch(request, env, ctx) {
        ...
        if (url.pathname === '/' + pAdmin || url.pathname === '/' + pAdmin + '/') {
            if (request.method === "POST") {
                try {
                    ...
                    if (cleanInput.startsWith(url.origin)) cleanInput = cleanInput.substring(url.origin.length);
                    let testPath = cleanInput.startsWith('/') ? cleanInput : '/' + cleanInput;

                    let pureUrl = inputUrl.trim();
                    const allPrefixes = [pAdmin, pShare, pRef, pSimp, pSho, pHid, pDcb, ...pUhm, ...pUem, 'gis', 'pas'];
                    ...
                    pureUrl = normalizeUrl(pureUrl);
                    let origin = url.origin;

                    let disUrl = `${origin}/${makeFullUrlPart(pureUrl, pShare)}`;
                    let links = { "明文直链反代 (DIS)": disUrl };

                    let regRes = parseRefRegistry(pureUrl);
                    if (regRes) {
                        let ctxRef = parseRouteContext('/' + pRef + '/' + pureUrl.replace(/^https?:\/\//i, ''), pRef.length, '');
                        links["托管精炼反代 (SIMP)"] = `${origin}/${pSimp}/${regRes.site.ref_URL.replace('://', '/')}${ctxRef.tail}`;
                        links["特征短链反代 (SHORT)"] = `${origin}/${regRes.site.aliasPrefix.replace(/^\//,'')}/${regRes.ID}${ctxRef.tail}`;
                    }

                    let routingPayload = makeFullUrlPart(pureUrl, pShare);
                    let b64Payload = btoa(routingPayload).replace(/=/g, '').replace(/\+/g, '-').replace(/\//g, '_');
                    
                    let tail = "";
                    if (pureUrl.toLowerCase().match(/\.(jpg|jpeg|png|gif|webp|ico|bmp|svg|avif)/)) {
                        let m = pureUrl.match(/\/([^\/]+\.[a-z0-9]+)$/i);
                        if (m) tail = '/' + m[1];
                    }
                    if (pureUrl.toLowerCase().endsWith('.b64')) tail = '/image.jpg';
        ...
        if (contentLength > maxBackendBytes) {
            return new Response(`Forbidden: Target file size exceeds backend limit (${CONFIG.BACKEND_MAX_SIZE_KB} KB).`, { status: 413 });
        }
        if (contentLength === -1 && CONFIG.ALLOW_BACKEND_CHUNKED_RAW === 0) {
            return new Response("Forbidden: Chunked streaming asset without content-length is blocked.", { status: 411 });
        }
                    ...
                    function push() {
                        reader.read().then(({ done, value }) => {
                            ...
                            if (totalBytesRead > maxBackendBytes) {
                                controller.error(new Error(`File size overflowed limit of ${CONFIG.BACKEND_MAX_SIZE_KB} KB during streaming.`));
                                return;
                            }
                            controller.enqueue(value);
                            push();
        ...
        try {
            ...
            if (upstreamText.length > maxBackendBytes * 1.35) {
                return new Response(`Forbidden: Base64 payload text size exceeds limit.`, { status: 413 });
            }
            ...
            if (bytes.length > maxBackendBytes) {
                return new Response(`Forbidden: Decoded binary data size (${Math.round(bytes.length/1024)} KB) exceeds backend limit.`, { status: 413 });
            }
...
<head>
    ...
    <style>
        ...
        input[type="text"] { flex: 1; padding: 14px; font-size: 14px; border: 1px solid #ccc; border-radius: 4px; box-sizing: border-box; outline: none; }
        input[type="text"]:focus { border-color: #409eff; }
        select.mode-select { padding: 12px; font-size: 14px; border: 1px solid #ccc; border-radius: 4px; background: #fff; outline: none; box-sizing: border-box; color: #333; }
        select.mode-select:focus { border-color: #409eff; }
        .btn-row { display: flex; gap: 12px; margin-bottom: 15px; }
        .btn { padding: 14px; font-size: 14px; color: #fff; background: #409eff; border: none; border-radius: 4px; cursor: pointer; font-weight: bold; text-align: center; box-sizing: border-box; }
        ...
        .link-item { margin: 8px 0; padding: 6px 0; border-bottom: 1px dashed #e4e7ed; display: flex; flex-direction: column; }
        .link-label { font-weight: bold; color: #409eff; font-size: 12px; margin-bottom: 2px; }
        .link-url { color: #303133; text-decoration: none; word-break: break-all; font-size: 13px; }
...
<body>
    <div class="box">
        ...
        <div class="btn-row">
            <select id="modeSelect" class="mode-select btn-flex">
            ...
            <button class="btn btn-purple btn-flex" id="btnRefine">生成反代链接</button>
            <button class="btn btn-green btn-flex" id="btnPick">图片转 Base64 编码</button>
        </div>

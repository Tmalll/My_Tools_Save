...
// CF 部署时建议留空, 减少path长度.
// NGX 部署时建议填写一个安全PATH, 让其聚合到一个PATH, 这样反代时就不用写多个反代规则了. 
// SP3eHm618kN71DD
    ...
    async fetch(request, env, ctx) {
            ...
            if (request.method === "POST") {
                try {
                    ...
                    if (regRes) {
                        ...
                        links["特征短链反代 (SHORT)"] = `${origin}/${regRes.site.shortName.replace(/^\//,'')}/${regRes.ID}${useTail}`;
                    }
        ...
        if (url.pathname.startsWith('/' + pRef + '/') || url.pathname.startsWith('/' + pSho + '/')) {
            ...
            if (!isRef) {
                ...
                if (/^[0-9a-f]{32}$/i.test(cleanId)) return Response.redirect(`${url.origin}${prefixGis}/${cleanId}${ctx.tail}`, 302);
                if (/^[a-zA-Z0-9]{8}$/.test(cleanId)) return Response.redirect(`${url.origin}${prefixPas}/${cleanId}${ctx.tail}`, 302);
            }
...
<body>
    ...
    <script>
        ...
        document.getElementById('btnRefine').onclick = function() {
            ...
            .catch(err => showUI(true, "网络异常：" + err.message));
        };

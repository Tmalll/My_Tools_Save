...
function normalizeUrl(urlStr) {
    ...
    return proto + '://' + raw.replace(/\/+/g, '/').replace('https:/gist.githubusercontent.com', 'gist.githubusercontent.com');
}
...
function handleRefAndShortRoute(url, pRef, pSho, pSimp, pShare) {
    ...
    return { redirect: true, url: `${url.origin}/${fallbackPart}`.replace(/\/+/g, '/') };
}
...
async function handleMainImageProxy(request, url, workingPath, pShare, ctx, browserHasImgTail) {
    ...
    try { upstreamUrl = new URL(upstreamUrlStr); } catch { return new Response("Invalid Upstream Target", { status: 400 }); }
    ...
    let reader = upstream.body.getReader(), { done, value: firstChunk } = await reader.read();
    if (done || !firstChunk) return new Response("Forbidden: Empty Stream Source.", { status: 403 });
    if (firstChunk.length > maxBackendBytes) return new Response(`Forbidden: Stream overflow.`, { status: 413 });
    ...
    if (!isRealImage) { try { reader.cancel(); } catch{} return new Response("Forbidden: Non-image assets blocked.", { status: 403 }); }
    if (!browserHasImgTail) { try { reader.cancel(); } catch{} return Response.redirect(`${url.origin}${url.pathname.replace(/\/+$/, '')}/image.${foundExt}${url.search}${url.hash}`, 302); }
    if (!isB64Mode) {
        let totalBytesRead = firstChunk.length;
        const ts = new TransformStream({
            start(controller) { controller.enqueue(firstChunk); },
            transform(chunk, controller) {
                totalBytesRead += chunk.length;
                if (totalBytesRead > maxBackendBytes) controller.error(new Error("Forbidden: Stream overflow."));
                else controller.enqueue(chunk);
            }
        });
        let bodyStream = upstream.body;
        if (bodyStream.locked) {
        ...
        return makeImageResponse(bodyStream.pipeThrough(ts), detectedMime, null, cacheKey, cache, ctx);
    }

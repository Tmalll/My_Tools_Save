...
// ==================== 📦 自动生成的前端控制面板 (Dual-Mode Advanced Config Edition) ====================
function getHelpHTML(origin) {
    ...
    // -----------------------------------------------------------------------------------------
    const PANEL_TITLE = "[ CF 万能反代脚本 2026-06-22 v3 ]";
    const IS_JUMP = 0; // 页面链接默认跳转模式, 0 = 当前标签打开, 1 = 跳转新标签打开

    const prefix = `${origin}${CONFIG.AUTH_PREFIX}`; 
    // 根据 IS_JUMP 决定页面上固有示例链接的打开行为，0 模式下为了保留抓包上下文全部在当前页打开
    ...
    <script>
        ...
        document.getElementById('btnGo').addEventListener('click', function(e) {
            ...
            // 完美符合直觉的按键捕获：如果用户按住了 Ctrl 键（Mac上为 Command 键），强行在新标签页打开链接

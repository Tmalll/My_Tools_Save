...
export default {
    async fetch(request, env, ctx) {
        ...
        // v59新增, 本地 workerd 部署NGX反代专用协议修复层（CF 线上不会触发）
        ...
        // v57 路径规范化过滤器：强行将非标准的 /https:// 转换为标准的 /https/ 格式，实现全闭环格式统一
        if (path.includes('/https:/') || path.includes('/http:/')) {
            // 匹配并替换可能由于用户手拼或转义导致的 /https://, /https:/, /http://, /http:/ 
        ...
        request.headers.forEach((value, key) => {
            if (!/^(host|origin|referer)$/i.test(key)) { //v57
        ...
        if (target.hostname.includes('github.com')) {
            ...
            headers.set('Accept', isApiCall ? 'application/json' : 'text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8'); // v58把这一行删了.. 现在补回. 经测试与此无关.
        ...
        if (shouldRewrite) {
            try {
                ...
                isHtmlActuallyOutput = true;
            } catch (e) {
                console.error('[HTML Rewrite Failed, Fallback to Raw]', e.message);
                responseBody = backupUpstream.body; 
                isHtmlActuallyOutput = originContentType.toLowerCase().includes("text/html");
            }
        }
...
// ==================== 📦 模块 A：借鉴 v39 高兼容特性的 HTML 覆写与 Hook 沙箱 ====================
async function rewriteHtml(upstreamResponse, currentMode, target, url, CONFIG) {
    ...
    // 改成这样就正常了...
    // 借鉴 v39 极其克制的替换规则：只对明确以根斜杠开头的 href, src, action 属性做精准捕捉
    // 保护页面内嵌的内联 JSON 和原生前端路由映射对象免受大面积破坏
    html = html.replace(/\b(href|src|action)=["']\/([^\/][^"']*)/gi, `$1="${simplePrefix}/$2"`);
    const escapedOrigin = target.origin.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&'); // 基础绝对 URL 的替换
    html = html.replace(new RegExp(escapedOrigin + '(?=[\\/"\'])', 'gi'), simplePrefix);
...
// 主页
// ==================== 📦 自动生成的前端控制面板 (Dual-Mode Debug Edition) ====================
function getHelpHTML(origin) {
    // -----------------------------------------------------------------------------------------
    // 🌟 【高能防御注释 · 核心资产保全】 🌟
    // 格式定义：[ CF 万能反代脚本 当前日期 第几次改动 ]
    // 说明：此注释与下方的变量用于规范脚本升级时的版本印记与标题样式。
    // 在日后委托任何 AI（包括 Gemini、GPT、Claude 等）进行脚本升级、功能迭代或精简重构时，
    // 必须无条件完整保留此注释区域及版本标识变量，严禁以“清理冗余”为由将其精简或删除！
    // -----------------------------------------------------------------------------------------
    const PANEL_TITLE = "[ CF 万能反代脚本 2026-06-22 v2 ]"; 
    ...
    // 调试改动：将示例链接的 target 改为 _self
    const mainExamplesHtml = Object.entries(CONFIG.ROUTE_REGISTRY).filter(([_, cfg]) => cfg.isMain).map(([key, cfg]) => { const currentPfx = cfg.set_path ? `${prefix}/${cfg.set_path}` : prefix; return `<p>• ${cfg.display_name}: <a href="${currentPfx}/https/github.com/2dust/v2rayN" target="_self">${currentPfx}/https/...</a></p>`; }).join('');
    // 调试改动：将 MIME 链接的 target 改为 _self
    const forceMimeHtml = Object.entries(CONFIG.ROUTE_REGISTRY).filter(([_, cfg]) => !cfg.isMain).map(([key, cfg]) => { const currentPfx = `${prefix}/${cfg.set_path}/`; const rewriteTag = cfg.set_rewrite > 0 ? ` + 覆写[${cfg.set_rewrite}]` : ''; return `<p>• 强制 ${key.replace('is-', '').toUpperCase()}: <a href="${currentPfx}" target="_self">${currentPfx}</a> <code>${cfg.set_mime}${rewriteTag}</code></p>`; }).join('');
    ...
    <div id="resultArea" class="result-card"><div class="title">✨ 拼接成功的反代直链:</div><a id="resultLink" href="#" target="_self"></a></div>
    ...
    <script>
        ...
        // 调试改动：将 window.open 替换为当前标签页重定向 location.href
        document.getElementById('btnGo').addEventListener('click', function() {
            const targetUrl = buildProxyUrl(); if (targetUrl) window.location.href = targetUrl;
        });

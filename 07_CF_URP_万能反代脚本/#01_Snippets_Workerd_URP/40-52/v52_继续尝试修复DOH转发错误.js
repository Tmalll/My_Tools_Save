...
const CONFIG = {
    ...
    ROUTE_REGISTRY: {
        ...
        'static':            { path: 'static',            mime: 'AUTO_MAP',               rewrite: 3, max_size: 10240, cache_ttl: 86400,  allow_doh: 0, isMain: true,  name: 'Static 覆写' },
        'static_no_rewrite': { path: 'static_no_rewrite', mime: 'AUTO_MAP',               rewrite: 3, max_size: 0,     cache_ttl: 604800, allow_doh: 0, isMain: true,  name: 'Static 长缓存' },
        'is-doh':            { path: 'is-doh',            mime: 'ORIGIN',                 rewrite: 3, max_size: 0,     cache_ttl: 300,    allow_doh: 1, isMain: true,  name: '强制 DOH (GET)' },
        'is-doh-post':       { path: 'is-doh-post',       mime: 'ORIGIN',                 rewrite: 3, max_size: 0,     cache_ttl: 0,      allow_doh: 2, isMain: true,  name: '强制 DOH (POST)' },
...
export default {
    async fetch(request, env, ctx) {
        ...
        if (CONFIG.DOH_PATHS.has(target.pathname)) {
            ...
            // 情况1：显式关闭 (allow_doh: 0)，严禁往下走普通反代，直接熔断
            if (allowed === 0) {
                return new Response("DOH Service Forbidden In This Mode", { status: 403 });
            }
            // 情况2：根据位掩码校验权限 (1: 仅GET, 2: 仅POST, 3: 全开)
...
async function handleDOH(request, target, ctx, globalTtl) {
    // return new Response("666777", { status: 405 });
    // return fetch(target.href); // 测试1 成功
    await cache.put(
        "https://a.com",
        new Response("1")
    );
    ...
    // return fetch(target.href); // 测试2, 成功

    const cache = caches.default;

    // return new Response(String(cache)); // 验证1 浏览器返回 [object Object]
    // return new Response(JSON.stringify(Object.getOwnPropertyNames(Object.getPrototypeOf(cache)))); // 验证2, 浏览器返回 ["add","addAll","delete","match","put","matchAll","keys","constructor"]
    //await cache.put(
    //    "https://a.com",
    //    new Response("1")
    //);
    //return fetch(target.href); // 验证3, 浏览器报错 Error 1202 Ray ID: a0e033f6594295ad • 2026-06-19 05:41:08 UTC Snippets exceeded subrequests limit
    ...
    // return fetch(target.href); // 测试3, 成功
    ...
    return fetch(target.href); // 测试4,  测试失败...

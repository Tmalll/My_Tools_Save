...
export default {
    async fetch(request, env, ctx) {
        ...
        request.headers.forEach((value, key) => {
            // 保留 accept-encoding，让 CF 与源站自行协商
        ...
        // 判决核心：允许进入重写的条件
        // 1. 有 Content-Length 且小于阈值 OR
        // 2. 没有 Content-Length（流式传输）
        // 只要不是"有明确超大 HTML"就尝试重写
        const shouldRewrite = CONFIG.ENABLE_HTML_REWRITE && isHtml && 
        ...
        if (shouldRewrite) {
            try {
                ...
                // 2. 替换源站的绝对路径（https://target.com/path → simplePrefix/path）
                html = html.replace(
                    new RegExp(target.origin.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&') + '(?=[\/"\'])', 'gi'),
                    simplePrefix
                );

                // 3. 替换完整的绝对 URL（https://target.com/path → 反代格式）
                // 这解决了跨域链接的问题
                html = html.replace(
                    /\b(href|src|action)=["'](https?:\/\/[^"']+)/gi,
                    (match, attr, urlStr) => {
                        try {
                            // 检查 URL 是否已经是反代格式（防止重复）
                ...
                // 如果 HTML 重写发生错误（比如内存爆炸），直接转发原始内容
...
async function handleDOH(request, target, url, ctx) {
    ...
    // DOH 缓存 Key 包含查询参数，防止不同 DNS 查询互相覆盖

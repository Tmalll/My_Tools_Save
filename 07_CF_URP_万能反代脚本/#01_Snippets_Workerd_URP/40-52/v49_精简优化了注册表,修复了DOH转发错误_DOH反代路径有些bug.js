...
const CONFIG = {
    ...
    ROUTE_REGISTRY: {
        // 核心运行模式 (主控面板展示项)
        'normal':            { path: '',                  mime: 'ORIGIN',                 rewrite: 2, max_size: 15360, cache_ttl: 1800,   allow_doh: 3, isMain: true,  name: '普通 (RAW)' },
...
export default {
    async fetch(request, env, ctx) {
        ...
        // 🟢 核心修复：在这里必须把当前的 url.search 完整带过去，保证 DOH 的 GET 请求携带上 ?dns= 核心参数
        ...
        // 🟢 DOH 路由分配与复合开关（allow_doh）权限判定
        if (currentMode.allow_doh > 0 || CONFIG.DOH_PATHS.has(target.pathname)) {
            const allowed = currentMode.allow_doh > 0 ? currentMode.allow_doh : 3; // 未明确指定则默认全开
        ...
        // 🟢 根据重构后的逻辑，进行全新多状态智能覆写判定
        let shouldRewrite = false;
        if (needsHtmlRewrite) {
            if (currentMode.rewrite === 3) {
                shouldRewrite = true; // 模式3：无视一切，强制覆盖
            } else {
                const contentLengthHeader = upstream.headers.get("content-length");
                if (contentLengthHeader !== null) {
                    const contentLength = parseInt(contentLengthHeader, 10);
                    if (contentLength <= currentMode.max_size * 1024) {
                        shouldRewrite = true; // 模式1和2：小于 max_size 则允许
                    }
                } else if (currentMode.rewrite === 2) {
                    shouldRewrite = true; // 模式2：在源站无长度标头时，允许尝试盲刷覆写
                }
            }
        }
...
// 🟢 彻底修复 DOH：内部请求路径带上完整的 href，保留所有参数

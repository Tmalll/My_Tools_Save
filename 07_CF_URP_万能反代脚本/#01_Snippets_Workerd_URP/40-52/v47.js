...
const CONFIG = {
    ...
    // ==================== 🛠️ 统一智能路由注册表 ====================
    // mime 选项: 'ORIGIN' (保持源站) | 'AUTO_MAP' (后缀映射) | 具体MIME字符串 (如 text/html)
    // max_size: 允许重写的最大文件大小 (单位: KB)。不覆写时此设置自动忽略。
    // rewrite_no_len: 当源站没有返回 Content-Length 标头时，是否允许尝试重写 (true/false)
    ROUTE_REGISTRY: {
        'normal': { path: '', mime: 'ORIGIN', rewrite: true, ttl: 1800, max_size: 15360, rewrite_no_len: true, github_fix: true, isMain: true, name: '普通 (RAW)' },
        'static': { path: 'static', mime: 'AUTO_MAP', rewrite: true, ttl: 86400, max_size: 10240, rewrite_no_len: true, github_fix: true, isMain: true, name: 'Static 覆写' },
        'static_no_rewrite': { path: 'static_no_rewrite', mime: 'AUTO_MAP', rewrite: false, ttl: 604800, max_size: 0, rewrite_no_len: false, github_fix: false, isMain: true, name: 'Static 长缓存' },
        'is-doh': { path: 'is-doh', mime: 'ORIGIN', rewrite: false, ttl: 300, max_size: 0, rewrite_no_len: false, github_fix: false, isMain: true, name: '强制 DOH 反代' },
        
        // 强制 MIME 模式扩展 (max_size 和 rewrite_no_len 均可在下方独立精细微调)
        'is-html': { path: 'is-html', mime: 'text/html', rewrite: true, ttl: 86400, max_size: 15360, rewrite_no_len: true, github_fix: false },
        'is-md': { path: 'is-md', mime: 'text/markdown', rewrite: false, ttl: 604800, max_size: 0, rewrite_no_len: false, github_fix: false },
        'is-markdown': { path: 'is-markdown', mime: 'text/markdown', rewrite: false, ttl: 604800, max_size: 0, rewrite_no_len: false, github_fix: false },
        'is-css': { path: 'is-css', mime: 'text/css', rewrite: false, ttl: 604800, max_size: 0, rewrite_no_len: false, github_fix: false },
        'is-js': { path: 'is-js', mime: 'application/javascript', rewrite: false, ttl: 604800, max_size: 0, rewrite_no_len: false, github_fix: false },
        'is-json': { path: 'is-json', mime: 'application/json', rewrite: false, ttl: 86400, max_size: 0, rewrite_no_len: false, github_fix: false },
        'is-txt': { path: 'is-txt', mime: 'text/plain', rewrite: false, ttl: 86400, max_size: 0, rewrite_no_len: false, github_fix: false },
        'is-xml': { path: 'is-xml', mime: 'text/xml', rewrite: false, ttl: 86400, max_size: 0, rewrite_no_len: false, github_fix: false },
        'is-csv': { path: 'is-csv', mime: 'text/csv', rewrite: false, ttl: 86400, max_size: 0, rewrite_no_len: false, github_fix: false }
    }
};
...
export default {
    async fetch(request, env, ctx) {
        ...
        if (currentMode.github_fix && target.hostname.includes('github.com')) {
...
function getHelpHTML(origin) {
    ...
    // 🟢 前端模板精简：移除了 cacheDesc 字段，且示例路径仅保留到 key/ 节点让用户自己拼接
    ...
    <script>
        ...
        document.getElementById('proxyForm').addEventListener('submit', function(e) {
            ...
            if(u.includes('${CONFIG.AUTH_PREFIX}')){ 
                ...
                // 递归清洗中间可能夹杂的任何标识符段 (如 /static/, /is-html/)

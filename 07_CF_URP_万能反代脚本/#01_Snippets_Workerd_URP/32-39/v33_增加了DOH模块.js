...
// --- DOH 模块配置 ---
const DOH_PATHS = new Set([
    '/dns-query',
    '/dns-query-7788' // 可以在这里继续自由追加自定义的 DOH 路径
]);
const Allow_POST_DOH = 0; // 1: 允许 POST DOH; 0: 禁止 POST DOH (防止高频无法缓存的请求消耗额度)

// 全局内存锁：用于拦截并合并短时间内完全相同的 DOH 并发请求
...
<head>
    ...
    <style>
        ...
        .notice{margin-top:30px;padding:12px;background:#fff3cd;color:#856404;border:1px solid #ffeeba;border-radius:4px;font-size:13px}
    </style>
</head>
<body>
    ...
    <div class="notice">
        ...
        本系统已集成 DOH (DNS over HTTPS) 反代与高速缓存模块。由于操作系统的安全 DNS、浏览器等设备会产生极高频、全天候不间断的解析请求，长期挂载 DOH 将会<b>极其快速地消耗</b>您 Cloudflare Workers 的每日免费请求额度。强烈建议仅作临时调试或紧急备用使用，否则可能因额度耗尽导致 Worker 停摆或触发官方限制。
    </div>
    ...
    <script>
        ...
        // --- DOH 模块核心拦截逻辑 ---
        const isDohPath = DOH_PATHS.has(target.pathname);
        if (isDohPath) {
            if (request.method === 'POST' && Allow_POST_DOH === 0) {
                return new Response("POST DOH Method Not Allowed", { status: 405 });
            }
            
            // 如果是 GET DOH 请求，尝试优先读取 Cloudflare 本地边缘缓存
            ...
            if (!upstreamPromise) {
                ...
                upstreamPromise = (async () => {
                    try {
                        ...
                        let userUA = request.headers.get('User-Agent') || 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/143.0.7499.87 Safari/537.36';
                        dohHeaders.set('User-Agent', userUA);
                        ...
                        return { status: res.status, headers: [], body: null };
                    } catch (err) {
                        return { status: 502, headers: [], body: null };
                    } finally {
                        // 请求完成或彻底出错后，立即从全局锁中移除它，释放内存
            ...
            // 并发进来的相同请求在此处静静等待上述同一个 Promise 的执行结果
            const result = await upstreamPromise;

            if (result.status === 200 && result.body) {
                const dohRespHeaders = new Headers();
                dohRespHeaders.set("Content-Type", "application/dns-message");
                dohRespHeaders.set("Access-Control-Allow-Origin", "*");
                
                // 强制下发边缘强缓存标头，缓存 30 秒（可根据需要自行调整生存期）
                if (request.method === 'GET') {
                    dohRespHeaders.set("Cache-Control", "public, max-age=30");
                } else {
                    dohRespHeaders.set("Cache-Control", "no-store");
                }

                const responseToReturn = new Response(result.body, { status: 200, headers: dohRespHeaders });
                
                // 成功抓取后，如果是 GET 请求，在返回的同时顺手写入一次 Cloudflare 边缘硬件缓存
                if (request.method === 'GET') {
                    await cache.put(request, responseToReturn.clone());
                }
                return responseToReturn;
            }
            return new Response("DOH Upstream Error", { status: result.status });
        }
        // --- DOH 模块结束 ---

...
export default {
    async fetch(request, env, ctx) {
        ...
        // 1. 【优化】精准拦截无用图标/配置请求，不再盲目通过后缀名过滤，防止误杀正常图片
        ...
        // 【优化】UA 兜底逻辑优化，避免对已有 UA 的二次覆盖，逻辑更漂亮
        ...
        // 【优化】进一步剥离更全面的安全响应头（新增 COOP 与 Origin-Agent-Cluster），彻底解放 iframe 限制
        ...
        // 7. 静态内容增强模式配置
        if (isStaticMode) {
            // 【优化】只有在状态码为 2xx 成功时才允许长时间缓存；404、502 等错误响应一律不缓存
        ...
        if (CONFIG.ENABLE_HTML_REWRITE && ct.includes("text/html")) {
            ...
            // 【优化】支持更全面的 HTML 标签属性替换（增加 srcset, poster, data-src）
...
async function handleDOH(request, target) {
    ...
    // 【优化】考虑到未来可能开启 POST DOH，如果存在 body 则对 body 计算哈希或简单标志并入锁名，防止并发请求冲撞
    let bodyHash = "";
    if (request.method === 'POST') {
        bodyHash = request.headers.get("content-length") || "post-doh";
    }
    const lockKey = `${request.method}:${target.href}:${bodyHash}`;

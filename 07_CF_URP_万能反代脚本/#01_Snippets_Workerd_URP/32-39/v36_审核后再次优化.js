...
export default {
    async fetch(request, env, ctx) {
        ...
        // 7. 静态内容增强模式配置
        if (isStaticMode) {
            if (upstream.status >= 200 && upstream.status < 300) { // 只有在状态码为 2xx 成功时才允许长时间缓存；404、502 等错误响应一律不缓存
                ...
                respHeaders.set("Cache-Control", "no-store, no-cache, must-revalidate, max-age=0"); // 异常状态码强制不缓存
            }
        }
...
async function handleDOH(request, target) {
    ...
    // 【优化】缓存键（Cache Key）改造：直接以上游真实的 DOH 完整 URL 作为 key，彻底解耦 Worker 前缀与域名
    ...
    if (result.status === 200 && result.body) {
        const headers = new Headers({
            ...
            "Access-Control-Allow-Origin": "*",
            // 【进阶方案五】通过全局变量配置灵活、可控的统一缓存 TTL 时间

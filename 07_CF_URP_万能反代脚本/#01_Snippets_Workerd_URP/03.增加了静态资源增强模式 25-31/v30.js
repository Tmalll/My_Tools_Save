...
<head>
    ...
    <style>
        body{font-family:-apple-system,sans-serif;max-width:750px;margin:40px auto;padding:0 20px;color:#333;line-height:1.6}
        ...
        button{padding:10px 20px;background:#0076ff;color:#fff;border:none;border-radius:0 4px 4px 0;cursor:pointer;font-weight:bold}
        .sample-title{font-size:13px;font-weight:bold;color:#444;margin-top:10px;margin-bottom:4px}
        .sample-box{font-size:13px;background:#f1f1f1;padding:12px;border-radius:4px;word-break:break-all;line-height:1.8}
        a{color:#0076ff;text-decoration:none;display:block}
...
<body>
    ...
    <div class="box">
        ...
        <div class="sample-box">
            <a href="${prefixNormal}/https://github.com/2dust/v2rayN" target="_blank">${prefixNormal}/https://github.com/2dust/v2rayN</a>
            <a href="${prefixNormal}/https/github.com/2dust/v2rayN" target="_blank">${prefixNormal}/https/github.com/2dust/v2rayN</a>
        </div>
    </div>
    ...
    <div class="box">
        ...
        <div class="sample-title">拼接示例:</div>
        <div class="sample-box">
            <a href="${prefixStatic}/https://gist.githubusercontent.com/Tmalll/74f08eaff7dfdb21f547a71af2596498/raw/7.10.html" target="_blank">${prefixStatic}/https://gist.githubusercontent.com/Tmalll/74f08eaff7dfdb21f547a71af2596498/raw/7.10.html</a>
            <a href="${prefixStatic}/https/gist.githubusercontent.com/Tmalll/74f08eaff7dfdb21f547a71af2596498/raw/7.10.html" target="_blank">${prefixStatic}/https/gist.githubusercontent.com/Tmalll/74f08eaff7dfdb21f547a71af2596498/raw/7.10.html</a>
        </div>
    </div>
    ...
    <script>
        function setupForm(formId, prefix) {
            document.getElementById(formId).addEventListener('submit', function(e) {
                ...
                // 去除可能已经 national 包含的前缀路径，提取真实目标
                if(u.includes('${AUTH_PREFIX}')){
                    u = u.split('${AUTH_PREFIX}')[1].replace(/^\\/static\\//i, '/').replace(/^\\//, '');
                }

                // 统一将输入框的带有 :// 的连接规范化成直接可以被后端解析的单斜杠格式，绕过 302 重定向拦截器
                if(/^https?:\\/\\//i.test(u)){

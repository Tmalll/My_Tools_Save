...
export default {
    async fetch(request) {
        ...
        if (path === '/' && search === '') {
            const link1 = `${url.origin}${AUTH_PREFIX}/https://github.com/2dust/v2rayN`;
            const link2 = `${url.origin}${AUTH_PREFIX}/https/github.com/2dust/v2rayN`;

            const dynamicHelpText = `<!DOCTYPE html><html><head><title>Proxy Usage</title><meta name="viewport" content="width=device-width, initial-scale=1"></head><body><h3>Please enter the target domain name.</h3><p>Example 1: <a href="${link1}">${link1}</a></p><p>Example 2: <a href="${link2}">${link2}</a></p></body></html>`;

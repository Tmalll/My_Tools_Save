    ...
    async fetch(request) {
        ...
        if (AUTH_PREFIX) {
            ...
            if (path === '/' + cleanPrefix || path === '/' + cleanPrefix + '/') {
                ...
                // 【绝对隐藏】未授权访问，直接引发运行时引用错误，迫使 Cloudflare 弹出纯正官方 1101 页面
        ...
        // 3. 渲染主页帮助文档（只有带对 AUTH_PREFIX 的合法请求，在被标准化规整 path = '/' 后才会走到这里）
        if (path === '/' && search === '') {
            const prefix = `${url.origin}${AUTH_PREFIX.replace(/\/$/, '')}`; 
            const targetUrl = 'https://github.com/2dust/v2rayN';
            const link1 = `${prefix}/${targetUrl}`;
            const link2 = `${prefix}/https/${targetUrl.replace('https://', '')}`;

            // 如果是你自己合法访问主页时发起的 HEAD 请求，依然给 200 兜底，确保你自己用的时候不卡死

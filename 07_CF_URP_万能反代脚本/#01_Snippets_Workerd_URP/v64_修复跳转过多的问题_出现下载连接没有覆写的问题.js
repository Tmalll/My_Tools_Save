...
export default {
    async fetch(request, env, ctx) {
        ...
        // 【🔥已修复】防止创建 target 时因为含有参数导致 trailing slash (尾部斜杠) 丢失，杜绝 301 重定向死循环
...
function getHelpHTML(origin) {
    const PANEL_TITLE = "CF 万能反代脚本 2026-07-08 (Fix 尾斜杠重定向版)";
    const IS_JUMP = 0; 
    ...
    <div class="notice"><strong> DOH 风险：</strong> 使用 CF-Wokers / Snippets 部署时, 反代 DOH 极易快速消耗响应次数, 导致项目被艹. 反代目标必须使用合法域名格式, 如: https://dns.google/dns-query，IP格式的可能会无法使用...</div>
    <script>

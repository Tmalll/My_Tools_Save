...
function getHtmlContent(urlList, workerBaseUrl) {
    ...
    const HIGH_SPEED_LIMIT_MBPS = 5;
    const HIGH_SPEED_LIMIT_BPS = HIGH_SPEED_LIMIT_MBPS * MB_PER_SECOND;
    ...
    const html = `
        ...
        <head>
            ...
            <style>
                ...
                #chunkContainer.disabled { opacity: 0.5; pointer-events: none; } /* 禁用时的样式 */
            </style>
        </head>
        <body>
            ...
            <form id="testForm">
                ...
                <div class="form-group" id="chunkContainer">
                    ...
                    <div class="note" id="chunkNote">当限速 > ${HIGH_SPEED_LIMIT_MBPS} MB/s 时，此设置将被忽略，块大小自动调整为 限速速率的 1/20。</div>
                </div>
            ...
            <script>
                document.addEventListener('DOMContentLoaded', () => {
                    ...
                    // 新增常量
                    const HIGH_SPEED_LIMIT_BPS = ${HIGH_SPEED_LIMIT_BPS}; // 5 MB/s 的字节数
                    ...
                    function createRateLimiter(limitBps, userChunkSizeB) {
                        const LIMIT_CHUNK_SIZE = userChunkSizeB; 
                    ...
                    // --- 核心多线程启动逻辑 ---
                    async function performSpeedTest(targetUrl, threads, limitBps, limitValue, limitUnit, userChunkSizeB, chunkValue, chunkUnit) {
                        ...
                        let actualChunkSizeB = userChunkSizeB;
                        let chunkInfo = \`\${chunkValue} \${chunkUnit}\`;

                        // 新的块大小逻辑
                        if (limitBps > HIGH_SPEED_LIMIT_BPS) {
                            actualChunkSizeB = Math.round(limitBps / 20); // 限速的 1/20 B
                            chunkInfo = \`自动计算: \${(actualChunkSizeB / MB).toFixed(3)} MB (限速的 1/20)\`;
                        } else if (limitBps === 0) {
                            actualChunkSizeB = 10 * MB; // 无限速时使用较大的块，避免不必要的延迟
                            chunkInfo = \`不限速，使用大块: \${(actualChunkSizeB / MB).toFixed(0)} MB\`;
                        }
                        ...
                        document.getElementById('resultDiv').innerHTML = \`
                            ...
                            <ul>
                                ...
                                <li>**限速块大小**: \${chunkInfo} (\${(actualChunkSizeB).toFixed(0)} 字节)</li>
                            </ul>
                        \`;
                        ...
                        for (let i = 1; i <= threads; i++) {
                            ...
                            setTimeout(() => {
                                if (isTestRunning) { 
                                    performSingleThreadTest(i, targetUrl, limitBps, actualChunkSizeB);
                                }
                            }, (i - 1) * 500); 
                        }
                    }
                     ...
                     * 单线程测速逻辑 
                     */
                    async function performSingleThreadTest(threadId, targetUrl, limitBps, actualChunkSizeB) {
                    ...
                    const chunkUnitSelect = document.getElementById('chunkUnit');
                     ...
                     * 根据限速值动态启用/禁用块大小输入框
                     */
                    function toggleChunkInputState() {
                        const limitValue = parseFloat(limitValueInput.value.trim());
                        const limitUnit = limitUnitSelect.value;
                        let unitMultiplier = limitUnit === 'MB' ? MB : KB;
                        const limitBps = limitValue * unitMultiplier;

                        if (limitBps > HIGH_SPEED_LIMIT_BPS) {
                            chunkContainer.classList.add('disabled');
                            chunkNote.textContent = \`✅ 自动调整: 限速 > ${HIGH_SPEED_LIMIT_BPS} MB/s，块大小自动调整为 限速速率的 1/20。\`;
                        } else {
                            chunkContainer.classList.remove('disabled');
                            chunkNote.textContent = \`请手动设置块大小。限速 > ${HIGH_SPEED_LIMIT_BPS} MB/s 时，此设置将被忽略，块大小自动调整为 限速速率的 1/20。\`;
                        }
                    }

                    // 绑定事件，实时检查
                    limitValueInput.addEventListener('input', toggleChunkInputState);
                    limitUnitSelect.addEventListener('change', toggleChunkInputState);
                    toggleChunkInputState(); // 首次加载时调用一次
                    ...
                    document.getElementById('testForm').addEventListener('submit', function(e) {
                        ...
                        const chunkValue = parseFloat(chunkValueInput.value.trim());
                        const chunkUnit = chunkUnitSelect.value;
                        ...
                        // 2. 计算用户指定的块大小 (Bytes)
                        let chunkMultiplier = chunkUnit === 'MB' ? MB : KB;
                        let userChunkSizeB = chunkValue * chunkMultiplier;
                        ...
                        // 3. 启动测试
                        // 注意：这里传入的是用户设置的原始 chunkValue/Unit，实际在 performSpeedTest 内部会根据限速值计算最终的 actualChunkSizeB
                        performSpeedTest(targetUrl, threads, limitBps, limitValue, limitUnit, userChunkSizeB, chunkValue, chunkUnit);
                    });

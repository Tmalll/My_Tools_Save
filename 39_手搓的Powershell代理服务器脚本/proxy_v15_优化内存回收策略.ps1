...
# 上游代理设置 (Outbound Proxy):
#    - "direct" : 本机直接发起连接
#    - "socks5://127.0.0.1:7890" : 转发给上游 SOCKS5 代理
#    - "http://127.0.0.1:10900"   : 转发给上游 HTTP 代理
$Outbound = "direct"
...
$MonitorInterval = 3000   # 内存回收间隔(ms), 你可以自由修改这里，例如 500, 2000, 10000
...
# ====================================================================
# 【核心高性能 Core v3.2.3】（已修复 HTTP 上游模式下的 IPv6 方括号 Bug）
# ====================================================================
$ProxyCoreSource = @'
using System;
using System.Net;
                ...
                if (!string.IsNullOrEmpty(targetHost) && targetPort > 0) {
                    ...
                    if (outboundType == "DIRECT") {
                        ...
                        try {
                            ...
                            string httpHostFormatted = targetHost;
                            if (targetHost.Contains(":") && !targetHost.StartsWith("[")) {
                                httpHostFormatted = string.Format("[{0}]", targetHost);
                            }

                            string connectHeader = string.Format("CONNECT {0}:{1} HTTP/1.1\r\nHost: {0}:{1}\r\nProxy-Connection: Keep-Alive\r\n\r\n", httpHostFormatted, targetPort);
...
[Console]::WriteLine("${esc}[32m  Dual Proxy Service (HTTP & SOCKS5 Pure .NET v3.2.3)${esc}[0m")
...
while ($true) {
    ...
    if ($logTimer.ElapsedMilliseconds -ge $MonitorInterval) {
        
        # 【核心回收逻辑】：无论 Debug 开关如何，只要无任务，就在后台默默回收内存

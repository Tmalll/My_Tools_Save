...
show_help() {
    echo -e "${GREEN}Base64 自动化处理工具 (v9.7)${NC}"
    ...
    echo "  -c, --check          执行详细双重校验（显示详细 Size+MD5 对比）"
...
do_encode() {
    ...
    if [[ "$no_hash" -eq 0 ]]; then
        ...
        local e_size=$(get_size "$final_out")
        local e_md5=$(get_md5 "$final_out")
        ...
        echo -e "${CYAN}    原始: $s_size|$s_md5${NC}"
        echo -e "${CYAN}    编码: $e_size|$e_md5${NC}"
    else
        echo -e "${YELLOW}  (已跳过校验生成)${NC}"
    fi
}
...
do_decode() {
    ...
    if [[ "$do_check" -eq 1 ]]; then
        echo -e "${PURPLE}正在校验编码文件: $fname${NC}"
        if [ -f "$md5_file" ]; then
            ...
            if [ "$saved_enc_info" == "$current_enc_info" ]; then
                echo -e "${CYAN}  - 预校验通过: $current_enc_info${NC}"
            else
                echo -e "${RED}  - 预校验不通过: 编码文件损坏！${NC}"
                echo -e "${RED}    当前: $current_enc_info${NC}"
                echo -e "${RED}    预期: $saved_enc_info${NC}"
                return # 批量模式下不 exit 1，而是跳过该文件处理下一个
            fi
        else
            echo -e "${RED}  - 跳过校验: 未找到 .md5 文件${NC}"
        fi
    fi
    ...
    if [[ "$do_check" -eq 1 && -f "$md5_file" ]]; then
        ...
        echo -e "${PURPLE}正在校验原始完整性:${NC}"
        if [ "$saved_ori_info" == "$current_ori_info" ]; then
            echo -e "${YELLOW}  - 源文件正常: $current_ori_info${NC}"
            echo -e "${GREEN}  ✓ 最终校验通过！${NC}"
        else
            echo -e "${RED}  ✗ 最终校验失败：内容不一致${NC}"
            echo -e "${RED}    当前: $current_ori_info${NC}"
            echo -e "${RED}    预期: $saved_ori_info${NC}"
        fi
    fi
}

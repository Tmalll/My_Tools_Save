...
clear
    ...
    echo -e "  -dq, --cd  (Current directory)  当前目录模式：扫描脚本所在当前目录" # 新增这行
    echo -e "                                   处理逻辑同 -a 自动模式"
    echo ""
    echo "---------- ---------- ---------- ---------- ----------"
    echo -e "${YELLOW}[手动模式]:"
...
# --- 路由主程序 ---
check_dependencies
SCRIPT_NAME=$(basename "$0") # 拿到脚本名，用于后续排除
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -dq|--cd) DIR_MODE=1; PATH_ARG="."; shift ;; # 新增：标记为目录模式，路径设为当前目录
...
if [[ $AUTO_SCAN -eq 1 || ($DIR_MODE -eq 1 && -d "$PATH_ARG") ]]; then
    ...
    # 核心修改：在 find 中增加 ! -name "$SCRIPT_NAME"
    find "$CURRENT_IN" -path "$CURRENT_OUT" -prune -o -type f ! -name "$SCRIPT_NAME" -print | while read -r f; do

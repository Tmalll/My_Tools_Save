...
>%~dp0.jobs\decode.ps1 echo param([string]$InFile,[string]$OutFile,[string]$JobFile)
>>%~dp0.jobs\decode.ps1 echo.
>>%~dp0.jobs\decode.ps1 echo $reader = [System.IO.StreamReader]::new($InFile,[System.Text.Encoding]::ASCII)
>>%~dp0.jobs\decode.ps1 echo $fs     = [System.IO.FileStream]::new($OutFile,[System.IO.FileMode]::Create)
>>%~dp0.jobs\decode.ps1 echo.
>>%~dp0.jobs\decode.ps1 echo while(-not $reader.EndOfStream){
>>%~dp0.jobs\decode.ps1 echo   $chunk = $reader.ReadLine()
>>%~dp0.jobs\decode.ps1 echo   if([string]::IsNullOrWhiteSpace($chunk)){ continue }
>>%~dp0.jobs\decode.ps1 echo   $bytes = [Convert]::FromBase64String($chunk)
>>%~dp0.jobs\decode.ps1 echo   $fs.Write($bytes,0,$bytes.Length)
>>%~dp0.jobs\decode.ps1 echo }
>>%~dp0.jobs\decode.ps1 echo.
>>%~dp0.jobs\decode.ps1 echo $reader.Close()
>>%~dp0.jobs\decode.ps1 echo $fs.Close()
>>%~dp0.jobs\decode.ps1 echo.
>>%~dp0.jobs\decode.ps1 echo Remove-Item $JobFile -Force
>>%~dp0.jobs\decode.ps1 echo Write-Host "解码完成: $InFile"

echo 脚本已生成
pause
goto :decodeLoop_start

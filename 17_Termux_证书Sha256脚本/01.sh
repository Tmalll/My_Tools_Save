...
get_sha256() {
    ...
    if [ "$type" = "file" ]; then
        ...
        out=$(openssl x509 -noout -fingerprint -sha256 -in "$target" 2>/dev/null)
        if [ $? -ne 0 ]; then out=$(openssl x509 -noout -fingerprint -sha256 -inform DER -in "$target" 2>/dev/null); fi
    else
        conn="$target"; if [[ "$target" != *:* ]]; then conn="$target:443"; fi
        echo -e "${BLUE}$target ↓${NC}"
        out=$(echo | openssl s_client -connect "$conn" 2>/dev/null | openssl x509 -noout -fingerprint -sha256 2>/dev/null)
    fi
    if [ -n "$out" ]; then
        hash=$(echo "$out" | cut -d'=' -f2 | tr -d ':' | tr '[:upper:]' '[:lower:]')

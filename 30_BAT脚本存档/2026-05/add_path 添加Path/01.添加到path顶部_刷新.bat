...
echo 添加到path列表顶部...
powershell -NoProfile -Command "$p='%ADD_PATH%';try{$r=[Microsoft.Win32.Registry]::LocalMachine.OpenSubKey('SYSTEM\CurrentControlSet\Control\Session Manager\Environment',$true);$o=$r.GetValue('Path','',[Microsoft.Win32.RegistryValueOptions]::DoNotExpandEnvironmentNames);if($o -split ';' -contains $p){Write-Host 'PATH 已存在，跳过。' -ForegroundColor Yellow}else{$n=if($o -and !$o.StartsWith(';')){\"$p;$o\"}else{\"$p$o\"};$r.SetValue('Path',$n,[Microsoft.Win32.RegistryValueKind]::ExpandString);Write-Host 'PATH 添加成功。' -ForegroundColor Green};$r.Close()}catch{Write-Host 'PATH 添加失败：'$_ -ForegroundColor Red}"
echo.

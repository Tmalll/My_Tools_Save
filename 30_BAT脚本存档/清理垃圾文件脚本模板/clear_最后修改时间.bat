...
:: ---- 第一条 PS 命令：删除 7 天前的文件 ----
powershell -NoProfile -Command "Get-ChildItem -Path '%TARGET%' -Recurse -File | Where-Object { $_.LastWriteTime -lt (Get-Date).AddDays(-7) } | Remove-Item -Force -ErrorAction SilentlyContinue"

...
:: 目标目录
set "Source1=E:\01.userData\ZhuoMian\工具存档"
set "TargetDIR3=\\192.168.1.120\e\#000-BakDIR\Github仓库\工具存档"
...
:: 目标目录
set "Source1=E:\01.userData\ZhuoMian\工具存档"
set "TargetDIR4=E:\01.userData\ZhuoMian\10.同步盘\OneDrive_个人\wutongskype@live.com\OneDrive\06.Github仓库\工具存档"
:: net use "%TargetDIR4%" "qwe123!!0952**" /user:"administrator" /persistent:no > nul 2>&1

:: echo ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- 
:: echo 去除 [ %TargetDIR4% ] 的重复文件...
:: rclone dedupe newest  "%TargetDIR4%" --by-hash --log-level ERROR
:: echo.

echo ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- 
echo 同步 [ %Source1% ] 到 [ %TargetDIR4% ] 开始...
echo. & echo.
rclone sync "%Source1%"  "%TargetDIR4%\01.latest" ^
    --backup-dir         "%TargetDIR4%\02.history\%timestamp%" ^
    --no-update-dir-modtime ^
    --no-update-modtime ^
    --modify-window 2s ^
    --fast-list ^
    ...
    --log-level INFO
echo.
echo 同步 [ %Source1% ] 到 [ %TargetDIR4% ] 完成...
timeout /t 1 > NUL
echo. & echo. & echo.
:: --log-level LogLevel  Log level DEBUG|INFO|NOTICE|ERROR (default NOTICE)
...
:: 目标目录
set "Source1=E:\01.userData\ZhuoMian\工具存档"
set "TargetDIR1=GD-MiaoSKY-YG:\Github仓库\工具存档"

echo ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- 
echo 去除 [ %TargetDIR1% ] 的重复文件...
...
:: 目标目录
set "Source1=E:\01.userData\ZhuoMian\工具存档"
set "TargetDIR2=OD-LiveSkype:\06.Github仓库\工具存档"

echo ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- 
echo 去除 [ %TargetDIR2% ] 的重复文件...
:: rclone dedupe newest  "%TargetDIR2%" --by-hash --log-level ERROR
echo.

echo 同步 [ %Source1% ] 到 [ %TargetDIR2% ] 开始...
echo. & echo.
rclone sync "%Source1%"  "%TargetDIR2%\01.latest" ^
    --backup-dir         "%TargetDIR2%\02.history\%timestamp%" ^
    --no-update-dir-modtime ^
    --no-update-modtime ^
    --modify-window 2s ^
    --fast-list ^
    ...
    --log-level INFO
echo.
echo 同步 [ %Source1% ] 到 [ %TargetDIR2% ] 完成...
timeout /t 1 > NUL
echo. & echo. & echo.
:: --log-level LogLevel  Log level DEBUG|INFO|NOTICE|ERROR (default NOTICE)

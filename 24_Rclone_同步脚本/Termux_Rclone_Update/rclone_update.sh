#!/bin/bash

# --- PID 锁紧凑版 ---
LOCK_FILE="${0}.lock"
exec 9>"$LOCK_FILE"
flock -n 9 || { echo "$(basename "$0") 已在运行中，退出"; exit 1; }
echo "$(basename "$0") 没有发现重复进程, 脚本开始运行... PID=$$"

sleep 5

# 清理缩略图缓存
rm -fr /storage/emulated/0/Pictures/.thumbnails/*
rm -fr /storage/emulated/0/Movies/.thumbnails/*

# 全局变量
DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"

# 代理设置
export http_proxy="socks5h://192.168.1.40:10800"
export https_proxy="$http_proxy"
export HTTP_PROXY="$http_proxy"
export HTTPS_PROXY="$http_proxy"

# 日志模块
init_log_top() {
    local name=$1
    SCRIPT_NAME="${0##*/}"
    SCRIPT_NAME="${SCRIPT_NAME%.sh}"    
    LOG_DIR="$(cd "$(dirname "$0")"; pwd)/${SCRIPT_NAME}_LOG"
    mkdir -p "$LOG_DIR"
    LOG_FILE="$LOG_DIR/$name.log"
    LOG_TMP="$LOG_DIR/$name.tmp"

    _finish() {
        exec 1>&- 2>&-; sleep 0.1
        [ -f "$LOG_TMP" ] && { 
            [ -f "$LOG_FILE" ] && find "$LOG_FILE" -mtime +7 -exec cp /dev/null {} \; 2>/dev/null
            [ -f "$LOG_FILE" ] && (cat "$LOG_TMP" "$LOG_FILE" > "$LOG_FILE.new" && mv "$LOG_FILE.new" "$LOG_FILE") || cp "$LOG_TMP" "$LOG_FILE"
            rm -f "$LOG_TMP"
        }
    }
    trap _finish EXIT; exec > >(tee "$LOG_TMP") 2>&1
}
# 调用示例: init_log_top "日志文件名称"
init_log_top "rclone_update_$(date '+%Y-%m-%d %H:%M:%S')"
# ...这里插入后续脚本...


echo "=========================================="
echo "任务启动时间: $(date '+%Y-%m-%d %H:%M:%S')"
echo "网络代理已设置: $http_proxy"
echo "=========================================="
echo ""

# 全局参数
COMMON_FLAGS="--modify-window 500ms --transfers 4 --checkers 4 --timeout 10s --contimeout 10s --retries 3 --low-level-retries 3 --log-level ERROR"
# --log-level LogLevel                  Log level DEBUG|INFO|NOTICE|ERROR (default NOTICE)
# --update 跳过目标上较新的文件

# 任务列表 --dry-run
task_list=(
    # 备份内置存储的文件...
    "sync, local:/storage/emulated/0, SMB-120:/E/#000-BakDIR/Phone-Rclone-BAK/ACE5-storage/#latest, \
    --backup-dir                      SMB-120:/E/#000-BakDIR/Phone-Rclone-BAK/ACE5-storage/oldbak_$(date +%Y-%m-%d[%H.%M.%S]) \
    --transfers 4 --checkers 4 \
    --one-file-system --copy-links \
    --exclude "**/.deleteRecord/**" \
    --exclude "**/BitComet/**" \
    --exclude "**/.thumbnails/**" \
    --exclude "**/.gs/**" \
    --exclude "**/.gs_fs0/**" \
    --exclude "**/.stx/**" \
    --exclude "**/.globalTrash/**" \
    --exclude "/.aaa/**" \
    --exclude "/.bbb/**" \
    --exclude "/.ccc/**" \
    --exclude "/Android/**" \
    --exclude "/MxPlayerAd/**" \
    --exclude "/storage/**" \
    --exclude "/*.log" --exclude "**/*.log" \
    --exclude "/*.lock" --exclude "**/*.lock" \
    --timeout 10s --contimeout 10s --retries 3 --low-level-retries 3 --log-level INFO"

    # 备份 Termux 核心文件...
    "sync, /data/data/com.termux, SMB-120:/E/#000-BakDIR/Phone-Rclone-BAK/ACE5-Termux_Backup/#latest, \
    --backup-dir                  SMB-120:/E/#000-BakDIR/Phone-Rclone-BAK/ACE5-Termux_Backup/oldbak_$(date +%Y-%m-%d[%H.%M.%S]) \
    --transfers 4 --checkers 4 \
    --one-file-system --copy-links \
    --exclude "/cache/**" \
    --exclude "/code_cache/**" \
    --exclude "/files/home/storage/**" \
    --exclude "/files/usr/bin/**" \
    --exclude "/files/usr/lib/**" \
    --exclude "/files/usr/share/**" \
    --exclude "/files/usr/include/**" \
    --exclude "/files/usr/var/cache/**" \
    --exclude "/files/usr/var/run/**" \
    --exclude "**/supervise/**" \
    --exclude "**/usr/var/log/**" \
    --exclude "/*.log" --exclude "**/*.log" \
    --exclude "/*.lock" --exclude "**/*.lock" \
    --exclude "/*.socket" --exclude "**/*.socket" \
    --exclude "/*.fifo" --exclude "**/*.fifo" \
    --exclude "/*.tmp" --exclude "**/*.tmp" \
    --exclude "/*.bash_history" --exclude "**/*.bash_history" \
    --timeout 10s --contimeout 10s --retries 3 --low-level-retries 3 --log-level INFO"

    #--exclude "/cache/**" \
    #--exclude "/code_cache/**" \
    #--exclude "/files/home/storage/**" \
    #--exclude "/files/usr/bin/**" \
    #--exclude "/files/usr/lib/**" \
    #--exclude "/files/usr/share/**" \
    #--exclude "/files/usr/include/**" \
    #--exclude "/files/usr/var/cache/**" \
    #--exclude "/files/usr/var/run/**" \
    #
    #--exclude "**/usr/var/log/**" \
    #--exclude "**/*.socket" \
    #--exclude "**/*.fifo" \
    #--exclude "**/*.log"
    #--exclude "**/*.tmp"
    #--exclude "**/*.bash_history"

    # 备份下载目录.
    #"sync, local:/storage/emulated/0/Download, SMB-120:/E/#14-Rclone-BAK/Note12-Download/#latest, \
    #--backup-dir SMB-120:/E/#14-Rclone-BAK/Note12-Download/oldbak_$(date +%Y-%m-%d[%H.%M.%S]) \
    #--transfers 4 --checkers 4 \
    #--one-file-system --copy-links \
    #--timeout 10s --contimeout 10s --retries 3 --low-level-retries 3 --log-level INFO"

    # 备份媒体数据 | 通话录音.
    #"sync, local:/storage/emulated/0/MIUI/sound_recorder, SMB-120:/E/#14-Rclone-BAK/Note12-sound_recorder/#latest, \
    #--backup-dir SMB-120:/E/#14-Rclone-BAK/Note12-sound_recorder/oldbak_$(date +%Y-%m-%d[%H.%M.%S]) \
    #--transfers 4 --checkers 4 \
    #--one-file-system --copy-links \
    #--timeout 10s --contimeout 10s --retries 3 --low-level-retries 3 --log-level INFO"

    # 备份媒体数据 | Movies.
    #"sync, local:/storage/emulated/0/Movies, SMB-120:/E/#14-Rclone-BAK/Note12-Movies/#latest, \
    #--backup-dir SMB-120:/E/#14-Rclone-BAK/Note12-Movies/oldbak_$(date +%Y-%m-%d[%H.%M.%S]) \
    #--transfers 4 --checkers 4 \
    #--one-file-system --copy-links \
    #--exclude "/.thumbnails/**" \
    #--timeout 10s --contimeout 10s --retries 3 --low-level-retries 3 --log-level INFO"

    # 备份媒体数据 | Music.
    #"sync, local:/storage/emulated/0/Music, SMB-120:/E/#14-Rclone-BAK/Note12-Music/#latest, \
    #--backup-dir SMB-120:/E/#14-Rclone-BAK/Note12-Music/oldbak_$(date +%Y-%m-%d[%H.%M.%S]) \
    #--transfers 4 --checkers 4 \
    #--one-file-system --copy-links \
    #--exclude "/.thumbnails/**" \
    #--timeout 10s --contimeout 10s --retries 3 --low-level-retries 3 --log-level INFO"

    # 备份媒体数据 | Pictures.
    #"sync, local:/storage/emulated/0/Pictures, SMB-120:/E/#14-Rclone-BAK/Note12-Pictures/#latest, \
    #--backup-dir SMB-120:/E/#14-Rclone-BAK/Note12-Pictures/oldbak_$(date +%Y-%m-%d[%H.%M.%S]) \
    #--transfers 4 --checkers 4 \
    #--one-file-system --copy-links \
    #--exclude "/.gs/**" \
    #--exclude "/.gs_fs0/**" \
    #--exclude "/.stx/**" \
    #--exclude "/.thumbnails/**" \
    #--timeout 10s --contimeout 10s --retries 3 --low-level-retries 3 --log-level INFO"

    # 备份媒体数据 | DCIM.
    #"sync, local:/storage/emulated/0/DCIM, SMB-120:/E/#14-Rclone-BAK/Note12-DCIM/#latest, \
    #--backup-dir SMB-120:/E/#14-Rclone-BAK/Note12-DCIM/oldbak_$(date +%Y-%m-%d[%H.%M.%S]) \
    #--transfers 4 --checkers 4 \
    #--one-file-system --copy-links \
    #--exclude "/.globalTrash/**" \
    #--exclude "/.deleteRecord/**" \
    #--timeout 10s --contimeout 10s --retries 3 --low-level-retries 3 --log-level INFO"

    # "local:/storage/emulated/0/backups,SMB-120:/E/#14-Rclone-BAK/backups-Note12,"
    # 备份到: 下载机E盘
    #"sync, local:/storage/emulated/0/backups, SMB-120:/E/#14-Rclone-BAK/Note12-backups/#latest, \
    #--backup-dir SMB-120:/E/#14-Rclone-BAK/Note12-backups/oldbak_$(date +%Y-%m-%d[%H.%M.%S]) \
    #--transfers 4 --checkers 4 \
    #--one-file-system --copy-links \
    #--exclude "**/*.log" --exclude "**/*.tmp" --exclude "**/*.bash_history" \
    #--timeout 10s --contimeout 10s --retries 3 --low-level-retries 3 --log-level INFO"

    # "local:/storage/emulated/0/backups,Home-WebDAV-BAK:/backups-Note12,"    
    # 备份到: Home-WebDAV
    #"sync, local:/storage/emulated/0/backups, Home-WebDAV-BAK:/Note12-backups/#latest, \
    #--backup-dir Home-WebDAV-BAK:/Note12-backups/oldbak_$(date +%Y-%m-%d[%H.%M.%S]) \
    #--transfers 4 --checkers 4 \
    #--one-file-system --copy-links \
    #--exclude "**/*.log" --exclude "**/*.tmp" --exclude "**/*.bash_history" \
    #--timeout 10s --contimeout 10s --retries 3 --low-level-retries 3 --log-level INFO"

    # "local:/storage/emulated/0/backups/Bak_To_JianGuoYun,WebDAV-JGY:/Note12,"    
    # 备份到: 坚果云
    #"sync, local:/storage/emulated/0/backups/Bak_To_JianGuoYun, WebDAV-JGY:/Note12/#latest, \
    #--backup-dir WebDAV-JGY:/Note12/oldbak_$(date +%Y-%m-%d[%H.%M.%S]) \
    #--transfers 4 --checkers 4 \
    #--one-file-system --copy-links \
    #--exclude "**/*.log" --exclude "**/*.tmp" --exclude "**/*.bash_history" \
    #--timeout 10s --contimeout 10s --retries 3 --low-level-retries 3 --log-level INFO"

    # 逻辑：备份整个 Termux 目录，追踪并拷贝软连接指向的真实文件，但排除掉指向手机存储的 storage 目录
    # "/data/data/com.termux,SMB-120:/E/Termux_Full_Backup,--copy-links --exclude files/home/storage/** --log-level INFO --transfers 32 --checkers 64 --dry-run"
    # "/data/data/com.termux,SMB-120:/E/Termux_Backup,--copy-links --exclude files/home/storage/** --exclude files/usr/lib/** --exclude files/usr/share/** --exclude files/usr/include/** --timeout 2s --retries 1 --low-level-retries 1 --ignore-errors"
    # 任务：Termux 全量备份
    # "/data/data/com.termux,SMB-120:/E/Termux_Backup,--dry-run \
    # --copy-links \
    # --exclude files/home/storage/** \
    # --exclude files/usr/lib/** \
    #　--exclude files/usr/share/** \
    # --exclude files/usr/include/** \
    # --timeout 2s \
    # --contimeout 2s \
    # --retries 1 \
    # --low-level-retries 1 \
    # --ignore-errors \
    # --log-level INFO"

    # 关于 --dry-run：如果你不确定 --exclude files/home/storage/** 写的路径对不对，可以在附加参数里加上 --dry-run。rclone 会模拟一遍过程并告诉你它会同步哪些、排除哪些，但不会真的写入数据。

    # 场景 A：只复制【小于】100MB 的文件 (排除大文件)
    # "local:/storage/emulated/0/backups,SMB-120:/E/BAK/small_files,--max-size 100M --skip-links"

    # 场景 B：只复制【大于】150MB 的文件 (专门备份大文件)
    # "local:/storage/emulated/0/backups,SMB-120:/E/BAK/big_files,--min-size 150M --skip-links"

    # 场景 C：只复制【100MB 到 500MB 之间】的文件 (区间筛选)
    # "local:/storage/emulated/0/backups,SMB-120:/E/BAK/range,--min-size 100M --max-size 500M"
    # --min-size SizeSuffix 仅传输大于此大小的文件（以 KiB 为单位）或后缀 B|K|M|G|T|P（默认关闭）
    # --max-size SizeSuffix 仅传输小于此值的文件（以 KiB 为单位）或后缀 B|K|M|G|T|P（默认关闭）

    # 示例 1：排除特定后缀（比如不备份大型视频或临时文件）
    # "local:/storage/emulated/0/backups,SMB-120:/E/#14-BAK/bk1,--exclude *.mp4 --exclude *.tmp"

    # 示例 2：排除软连接（如果全局带了 --links，想关掉它需要用 --ignore-links）
    # 注意：rclone 默认就是跳过软连接的，除非你显式加了 --links。
    # "local:/storage/emulated/0/backups,SMB-120:/E/#14-BAK/bk2,--ignore-links"
    # 参数,作用
    # (默认),跳过软连接，但在日志中显示 Can't follow symlink 的 Warning。
    # --skip-links,跳过软连接，并且关闭相关的警告日志。
    # --links,把软连接当做 .rclonelink 符号文件同步（通常用于保留链接关系）。
    # --copy-links,追踪软连接，把链接指向的真实文件/文件夹拷贝过去。

    # 示例 3：排除隐藏文件或特定文件夹（比如排除 .git 或 Android 的缓存目录）
    # "local:/storage/emulated/0/backups,WebDAV-JGY:/Note12,--exclude .*/** --exclude Android/data/**"

    # 示例 4：限制单任务速度（比如某些 WebDAV 服务限速，防止被封）
    # "local:/storage/emulated/0/backups,WebDAV-JGY:/Note12,--bwlimit 500k"

    # 示例 5：空参数（按默认 COMMON_FLAGS 执行）
    # "local:/storage/emulated/0/backups,Home-WebDAV-BAK:/backups-Note12,"

    # 场景 1：最干净的跳过软连接（不报警告）+ 排除特定后缀
    # "local:/storage/emulated/0/backups,SMB-120:/E/BAK/bk1,--skip-links --exclude *.tmp"

    # 场景 2：只同步文档，排除所有图片和视频
    # "local:/storage/emulated/0/backups,Home-WebDAV-BAK:/backups-Note12,--include *.{pdf,doc,docx,txt} --exclude *"

    # 场景 3：常规备份（无特殊参数，保持最后一个逗号或留空）
    # "local:/storage/emulated/0/backups/Bak_To_JianGuoYun,WebDAV-JGY:/Note12,"

    # 场景 4：同步工作目录，排除缓存文件夹和隐藏文件
    # "local:/storage/emulated/0/work,Home-WebDAV-BAK:/work_sync,--exclude .cache/** --exclude .*/** --skip-links"
    
    # 场景 5：镜像同步但限制单任务上传带宽（不影响其他任务）
    # "local:/storage/emulated/0/photos,SMB-120:/E/Photos,--bwlimit 2M"

)

# ================= 4. 执行逻辑 =================
echo " ========== ========== >>> 开始处理任务队列 <<< ========== ========== "

if [ ${#task_list[@]} -eq 0 ]; then
    echo "没有配置任何任务，退出。"
else
    for item in "${task_list[@]}"; do
        # 清理多余的换行符和反斜杠
        clean_item=$(echo "$item" | tr -d '\\' | tr '\n' ' ' | sed 's/  */ /g')
        
        # 解析参数
        cmd=$(echo "$clean_item" | cut -d',' -f1 | xargs)
        src=$(echo "$clean_item" | cut -d',' -f2 | xargs)
        dst=$(echo "$clean_item" | cut -d',' -f3 | xargs)
        extra=$(echo "$clean_item" | cut -d',' -f4- | xargs)

        echo "---------- ---------- ---------- ---------- ----------"
        echo "动作: [${cmd^^}]"  # 转换为大写显示
        echo "源端: $src"
        echo "目标: $dst"
        
        if [ -n "$extra" ]; then 
            echo "参数: $(echo "$extra" | sed 's/ --/\n      --/g')"
        fi
        
        # 转换为数组以正确处理带空格的参数
        read -ra EXTRA_ARGS <<< "$extra"
        read -ra COMMON_ARGS <<< "$COMMON_FLAGS"
        
        # 执行 rclone 命令
        rclone "$cmd" "$src" "$dst" "${COMMON_ARGS[@]}" "${EXTRA_ARGS[@]}"
        
        if [ $? -eq 0 ]; then
            echo "RESULT: SUCCESS - 任务完成"
        else
            echo "RESULT: ERROR - 任务失败，请查阅上方日志"
        fi
        echo ""
    done
fi

echo "=========================================="
echo "全部任务处理完毕: $(date '+%Y-%m-%d %H:%M:%S')"
echo "=========================================="
rm $LOCK_FILE

